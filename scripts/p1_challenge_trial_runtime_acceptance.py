#!/usr/bin/env python3
"""P1 Challenge Trial (chal4) runtime acceptance driver (issue #567).

Consumes the landed #649 guarded runtime fixture package and the #52 campaign
contract READ-ONLY (never duplicated or edited). Observes the chal4 guarded
boot, then grades the six arena gates honestly: PASS only where a natural
uninterrupted observation exists, UNTESTED otherwise. A captain-down run can
never substantiate a PASS.

Trial is separately known to hit a pre-existing PIKI BIRTH limitation: if the
guarded run cannot reach the stage, this driver records the exact blocker with
its log lines and the named shared owner instead of claiming a pass.
"""
import argparse
import hashlib
import json
...
SCHEMA = "p2-challenge-trial-acceptance-1"
KEY = LANE
GENERATION = 2

LAYOUT_KEY = "challenge:trial"
NATIVE_AREA_ID = 4
STAGE_INFO_INDEX = 20
    ...
    for stage in stages:
        if stage.get("slot") == STAGE:
            if stage.get("challenge_level") != LEVEL:
                return None, fail("chal4-level-mismatch")
            argv = stage.get("argv") or []
            if argv[-2:] != ["--experimental-challenge-level", str(LEVEL)]:
                return None, fail("chal4-argv-mismatch")
            return stage, None
    return None, fail("chal4-stage-missing")
...
def parse_run_log(text):
    ...
    facts["birth_limitation_suspected"] = (facts["window_960x540"]
                                           and not facts["fixture_pass_marker"]
                                           and facts["squad_pikis"] is None)
    return facts
...
def birth_limitation_record(facts, log_text):
    """Exact blocker record when the guarded run cannot reach the stage.

    Emitted only when the engine reached its window but no trial boot/squad
    marker appeared (the known pre-existing PIKI BIRTH limitation). Names the
    shared owner and quotes the exact log lines that evidence the stall.
    """
    if not facts.get("birth_limitation_suspected"):
        return None
    ...
    return {
        ...
        "stage": STAGE,
        "stage_key": LAYOUT_KEY,
        "detail": ("Engine reached its window/init but no %s squad/boot marker was "
...
def accept_facts(facts, guard_sha):
    ...
    spawn_ok = (facts["layout_id"] == "challenge-4"
    ...
    return {
        ...
        "issue": ISSUE,
        "stage": STAGE,
        "stage_contract": {"level_key": LAYOUT_KEY, "native_area_id": NATIVE_AREA_ID,
                           "stage_info_index": STAGE_INFO_INDEX, "source": SOURCE_INI},
        "facts": facts,
        "gates": gates,
...
def do_accept(args, out):
    ...
    limitation = birth_limitation_record(facts, text)
    if limitation is not None:
        record["blocked_reason"] = limitation
    ...
    print("gates_pass=%d gates_untested=%d captain_down=%s birth_limitation=%s" % (
        n_pass, 6 - n_pass, facts["captain_down"], limitation is not None))
    return 0
