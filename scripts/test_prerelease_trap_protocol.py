...
from randomizer.benefits import PRERELEASE
from randomizer.catalog import ITEM_IDS
from randomizer.seed import generate
from randomizer.session import Session, atomic_write
from randomizer.runner import NativeRun
for bits in range(8):
    bombs,traps,progg=bits&1,(bits>>1)&1,(bits>>2)&1
    for combined in (False,True):
        with tempfile.TemporaryDirectory() as d:
            m=generate('ambush','ap',permanent_checks=True,prerelease_trap_weight=1,progg_trap_weight=progg,bomb_trap_weight=traps,bomb_rock_weight=bombs,combined_captain=combined)
            s=Session(m,d);s.bind_ap('room',0,1)
            def run(expected,save=False,corrupt=False):
                r=NativeRun(s);r.write_state(True)
                assert f'BENEFITS {17+bombs+2*combined+4*traps+8*progg}\n' in r.bootstrap.read_text()
                if corrupt:
                    state=r.directory/'state.txt'
                    atomic_write(state,state.read_text().replace('BENEFITS 0 0 0 0 0 0 0 0 3','BENEFITS 0 0 0 0 0 0 0 0 1'))
            ...
            run(0)
            s.receive(0,[ITEM_IDS[PRERELEASE]]*2);s.receive(0,[ITEM_IDS[PRERELEASE]]*2)
            run(2) # unsaved effects replay on restart, like other consumables
            run(2,save=True)
            s=Session(m,d);run(0)
            s.receive(2,[ITEM_IDS[PRERELEASE]])
            run(1,save=True);run(0);run(0,corrupt=True)
print('PASS trap duplicate receipts, Prerelease modes 17-32, unsaved rollback, saved consumption, reload, retraction rejection')
