#!/usr/bin/env python3
"""Private leased build/run helper for the Mar attackable-range fixture (#683).

Drives the maintained generic fixture builder
(scripts/build_pikmin2_fixture.py) for native/tools/p2_mar_attackable_range_fixture.cpp,
then runs the guarded damage observation and validates markers. The consumer
fixture (p2_muse_mar_fixture.cpp, #375) is never touched.

Usage (from the canonical root):
  py -3.12 scripts/build_p2_mar_attackable_range.py --source <native> --build <build>
...
fixture provenance `built`, guarded run asserting PASS P2_MAR_RANGE_RUN with
natural damage markers and no CAPTAIN_DOWN / injection markers. Heavy jobs
require a canonical registry build lease held beforehand; this script never
...
build.ninja; `$in` = explicit objects + implicit libs in edge order) and then
delegates to the maintained pipeline unchanged: same freshness checks, same
input/toolchain snapshots, same provenance schema. The expansion is recorded
as `pikmin_pc.expanded.rsp` beside the provenance. No production rebuild.
...
FIXTURE = os.path.join("tools", "p2_mar_attackable_range_fixture.cpp")
PASS_MARKER = "PASS P2_MAR_RANGE_RUN"
DAMAGE_MARKER = "P2_MAR_RANGE_DAMAGE"
READY_MARKER = "P2_MAR_RANGE_READY"
...
def check_run_markers(text):
    """PASS + natural damage present, no captain-down, no injection."""
...
def main(argv=None):
    ...
    def absolute_with_path_lookup(value, root):
        """absolute() resolves bare program names against the build dir, which
        breaks when CMakeCache stores a bare compiler name (observed:
        CMAKE_CXX_COMPILER=g++). Fall back to a PATH lookup for bare names
        only, so the snapshot records the real binary Ninja invokes
        (C:/msys64/mingw64/bin/g++.exe per native-commands.txt). Absolute and
        slash-bearing paths resolve exactly as before."""
        from pathlib import Path
    ...
    for base in (args.output, args.build):
        for name in ("fixture.exe", "p2_mar_attackable_range.exe"):
    ...
    with open(os.path.join(args.output, "mar-range-run.log"), "w",
              encoding="utf-8", errors="replace") as f:
        f.write(out)
    record["run_log"] = os.path.join(os.path.abspath(args.output), "mar-range-run.log")
    record["run_log_sha256"] = sha256_file(record["run_log"])
    print(detail)
    with open(os.path.join(args.output, "mar-range-run.json"), "w",
              encoding="utf-8") as f:
