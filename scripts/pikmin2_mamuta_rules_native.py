"""Private Mamuta P2-rules native arena runner (batch 4, #221).
...
burial, and reset disabling the rules.
"""
import argparse
import json
import os
...
def validate(text):
    if 'PASS P2_MAMUTA_RULES_RUNTIME bury flower_stage cap99 navi_damage5 reset' not in text:
    ...
    return dict(squad=int(rows[0][2]), forced_buries=int(bury[0][0]),
                ...
                planted_count=int(planted[0][0]), reset=True)
