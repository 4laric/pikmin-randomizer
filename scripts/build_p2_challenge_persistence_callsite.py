#!/usr/bin/env python3
"""Private leased build/run helper for the challenge persistence CALLSITE fixture (#718).

Drives the maintained generic fixture builder
(scripts/build_pikmin2_fixture.py) for
native/tools/p2_challenge_persistence_callsite_fixture.cpp, then runs the guarded
...
Usage (from the canonical root):
  py -3.12 scripts/build_p2_challenge_persistence_callsite.py --source <native> --build <build>
...
P2_CHALLENGE_PERSISTENCE_CALLSITE_RUN with the exact 7 probe markers and no
...
FIXTURE = os.path.join("tools", "p2_challenge_persistence_callsite_fixture.cpp")
STAGE = "ch_MAT_route_rover"
PASS_MARKER = "PASS P2_CHALLENGE_PERSISTENCE_CALLSITE_RUN"
...
def main(argv=None):
    ...
    for base in (args.output, args.build):
        for name in ("fixture.exe", "p2_challenge_persistence_callsite.exe"):
    ...
    with open(os.path.join(args.output, "persistence-callsite-run.log"), "w",
              encoding="utf-8", errors="replace") as f:
        f.write(out)
    record["run_log"] = os.path.join(os.path.abspath(args.output), "persistence-callsite-run.log")
    record["run_log_sha256"] = sha256_file(record["run_log"])
    print(detail)
    with open(os.path.join(args.output, "persistence-callsite-run.json"), "w",
              encoding="utf-8") as f:
