"""Run the real-game prerelease fixture in an isolated session."""
import argparse
import os
...
from randomizer.benefits import PRERELEASE, DELIVERY
from randomizer.session import Session
from randomizer.runner import NativeRun

parser = argparse.ArgumentParser()
for name in ('exe', 'assets', 'output'): parser.add_argument('--'+name, type=Path, required=True)
parser.add_argument('--areas', nargs='+', default=['navel', 'spring'])
args = parser.parse_args()
for case in args.areas:
    m = generate('prerelease-regression', 'ap', collection_checks=True, starting_flarlic=1, prerelease_trap_weight=1, starting_area=case)
    ...
    env.pop('PRERELEASE_FIXTURE_LONG', None)
    if case == 'forest': env['PRERELEASE_FIXTURE_LONG'] = '1'
    ...
    with log.open('w', encoding='utf-8') as stream:
        ...
        try:
            ...
            while p.poll() is None and time.monotonic() < deadline:
                ...
                if 'PRERELEASE_PAUSED_READY' in text: session.receive(0, [ITEM_IDS[PRERELEASE]])
                run.write_state(True); time.sleep(.1)
            text = log.read_text(encoding='utf-8', errors='replace')
            assert p.poll() == 0 and 'PASS prerelease' in text, str(log)
            print(case, next(l for l in text.splitlines() if l.startswith('PASS prerelease')), flush=True)
        finally:
