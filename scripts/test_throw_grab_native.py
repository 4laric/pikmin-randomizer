"""Run the real-game quick-release fixture in isolated near/far sessions."""
import argparse
import os
...
for case in ('near', 'far'):
    m = generate('grab-regression', 'ap', collection_checks=True, starting_flarlic=1)
    ...
    with log.open('w', encoding='utf-8') as stream:
        ...
        try:
            ...
            assert p.poll() == 0 and 'PASS released-A grab then throw: flying=1' in text, str(log)
            print(case, next(l for l in text.splitlines() if l.startswith('PASS released-A')), flush=True)
        finally:
