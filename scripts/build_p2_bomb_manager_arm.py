"""Private leased build/run helper for the PORT-SIDE Bomb birth arm (#684)."""
import argparse
import hashlib
import json
import subprocess
...
def build_fixture(native, output, compiler='g++'):
    """Compile the fixture + provider + blast TU: tracked files only."""
    native, output = Path(native), Path(output)
    output.mkdir(parents=True, exist_ok=True)
    exe = output / 'p2_bomb_manager_arm_fixture.exe'
    cmd = [compiler, '-std=c++17', '-Wall', '-Wextra', '-Werror',
           '-I' + str(native / 'pc_port'),
           '-DP2_BOMB_MGR_BIRTH_NO_HOST',
           str(native / 'tools/p2_bomb_manager_arm_fixture.cpp'),
