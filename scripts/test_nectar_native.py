"""Run the real-game nectar fixture in an isolated session."""
import argparse
import os
...
from randomizer.benefits import FLOWERS
...
for case in ('nectar',):
    m = generate('nectar-regression', 'ap', collection_checks=True, starting_flarlic=1, bomb_trap_weight=0)
    ...
    with log.open('w', encoding='utf-8') as stream:
        ...
        try:
            ...
            while p.poll() is None and time.monotonic() < deadline:
                ...
                if 'NECTAR_PAUSED_READY' in text: session.receive(0, [ITEM_IDS[FLOWERS]])
                run.write_state(True); time.sleep(.1)
            text = log.read_text(encoding='utf-8', errors='replace')
            assert p.poll() == 0 and 'PASS nectar' in text, str(log)
            print(case, next(l for l in text.splitlines() if l.startswith('PASS nectar')), flush=True)
        finally:
