...
def run_exe(exe, timeout=300, path_prepend=None):
    env = dict(os.environ)
    env["PATH"] = ("C:/msys64/mingw64/bin;"
                   + (str(path_prepend) + ";" if path_prepend else "")
                   + env.get("PATH", ""))
    proc = subprocess.run(
...
MODULE_REL = Path("pc_port/pc_p2_challenge_mode.cpp")


def wrapper_source(native):
    """Private replacement-main wrapper (#656).

    The #651 fixture consumes module symbols defined in
    pc_p2_challenge_mode.cpp, which no CMake target compiles (shared
    registration is the pending #186 decision). Rather than edit #651 fixture
    or CMake, this wrapper includes the module TU and then the fixture TU so
    one compilation carries both -- the same accepted "module TU rides along
    via include" technique the integrated #672 boot fixture uses. Included
    files are read-only and enter the builder fixture-input snapshot.
    """
    native = Path(native).resolve()
    module_h = (native / "pc_port/pc_p2_challenge_mode.h").resolve()
    module_cpp = (native / MODULE_REL).resolve()
    fixture = (native / FIXTURE_REL).resolve()
    for path in (module_h, module_cpp, fixture):
        if not path.is_file():
            raise ValueError("Missing include input: " + str(path))
    return ("// generated by build_p2_challenge_mode_fixture.py (#656)\n"
            '#include "%s"\n' % str(module_h).replace(chr(92), "/")
            + '#include "%s"\n' % str(module_cpp).replace(chr(92), "/")
            + '#include "%s"\n' % str(fixture).replace(chr(92), "/"))
...
def phase_fixture(native, build, output, expected_head, guard_dir, log_path):
    ...
    wrapper = output / "challenge-mode-wrapper.cpp"
    wrapper.parent.mkdir(parents=True, exist_ok=True)
    wrapper.write_text(wrapper_source(native), encoding="utf-8")
    fixture = wrapper
