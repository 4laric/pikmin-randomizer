#!/usr/bin/env python3
"""Private leased build/run helper for the flora hookup fixture (#723).

Drives the maintained generic fixture builder
(scripts/build_pikmin2_fixture.py) for
native/tools/p2_flora_hookup_fixture.cpp, then runs the guarded hookup
observation and validates the hook markers. No maintained,
CMakeLists, preview, consumer (#561), or call-site (#710) edits. The fixture
proves the ENGINE CALL SITE in pc_port/pc_bbft.cpp emits 7/7 markers.

Usage (from the canonical root):
  py -3.12 scripts/build_p2_flora_hookup.py --source <native> --build <build>
...
P2_FLORA_HOOKUP_RUN with the hook markers and no
...
FIXTURE = os.path.join("tools", "p2_flora_hookup_fixture.cpp")
PASS_MARKER = "PASS P2_FLORA_HOOKUP_RUN"
def check_run_markers(text):
    """PASS + hook markers present, no captain-down, no injection."""
    ...
    for token in ("P2_FLORA_HOOKUP_ADMIT", "P2_FLORA_HOOKUP_CONVERT",
                  "P2_FLORA_HOOKUP_SPROUT", "P2_FLORA_HOOKUP_SCENERY",
                  "P2_FLORA_HOOKUP_DONE"):
        if token not in text:
            return False, "hook marker absent: " + token
    injected = [t for t in INJECTED_TOKENS if t in text]
    if injected:
        return False, "injection markers present: " + ",".join(injected)
    return True, "run PASS with hook markers"
...
def main(argv=None):
    ...
    for base in (args.output, args.build):
        for name in ("fixture.exe", "p2_flora_hookup.exe"):
    ...
    with open(os.path.join(args.output, "flora-hookup-run.log"), "w",
              encoding="utf-8", errors="replace") as f:
        f.write(out)
    record["run_log"] = os.path.join(os.path.abspath(args.output), "flora-hookup-run.log")
    record["run_log_sha256"] = sha256_file(record["run_log"])
    print(detail)
    with open(os.path.join(args.output, "flora-hookup-run.json"), "w",
              encoding="utf-8") as f:
