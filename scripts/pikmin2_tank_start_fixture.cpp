...
#include "Generator.h"
#include <cstring>
static bool tankStartPending=false,tankStartDown=false;
void tank_diag_pump_start(){if(tankStartPending){tankStartPending=false;tankStartDown=true;pc_bbft_start_button(true);std::fprintf(stderr,"TANK_START_EDGE delivered=1\n");}else if(tankStartDown){pc_bbft_start_button(false);tankStartDown=false;std::fprintf(stderr,"TANK_START_RELEASE\n");}}
class TankActorFixture:public PlugPikiApp {
 int frames=0,ready=0,moving=0,resetFrame=0;Teki* actor=nullptr;Teki* control=nullptr;Vector3f origin;float farthest=0;bool resetDone=false,finish=false,startSent=false;
public:
 int idle() override {
  int result=PlugPikiApp::idle();require(++frames<3600,"Tank actor timeout");
  if(gameflow.mMoviePlayer&&gameflow.mMoviePlayer->mIsActive){
   if(ready==0)gameflow.mMoviePlayer->requestSkip();
   if(ready>0&&!startSent&&SDL_getenv("TANK_DIAG_START")){
    for(MovieInfo* info=static_cast<MovieInfo*>(gameflow.mMoviePlayer->mPlayInfoList.mChild);info;info=static_cast<MovieInfo*>(info->mNext)){
     if(info->mName&&std::strstr(info->mName,"demo56")){startSent=true;tankStartPending=true;std::fprintf(stderr,"TANK_START_SCHEDULE movie=%s ready=%d\n",info->mName,ready);break;}
    }
   }
   return result;
  }
