"""Build the bomb joint capture fixture against a private build (#700).

Thin wrapper over scripts/build_pikmin2_fixture.py: links
native/tools/p2_bomb_joint_capture_fixture.cpp against the private
...
FIXTURE_REL = Path('tools/p2_bomb_joint_capture_fixture.cpp')
...
def run_guard_checks(exe, build):
    ...
    if selftest.returncode != 0 or 'P2_BOMB_JOINT_CAPTURE_SELFTEST_PASS' not in selftest.stdout:
