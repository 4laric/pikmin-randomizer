...
static bool snowRegistered(Teki* actor) {
    const char* name=pc_p2_enemy_name(static_cast<PelletView*>(actor));
    return name && !std::strcmp(name,"Snow Bulborb");
}
...
{
    if (!ok) { std::printf("FAIL Snow generated route: %s\n", why); std::fflush(nullptr); std::_Exit(1); }
}
    ...
    int idle() override
    {
        ...
        if (reloading) {
            ...
            Iterator all(tekiMgr); CI_LOOP(all) {
                ...
                if (snowRegistered(actor)) ++registered;
                if (actor->mGenerator && pc_randomizer_p2_bound_source(actor->mGenerator)==45) ++bound;
            }
            countPiki(&live);
            if (!postReloadFrames) std::printf("P2_REENTRY_COUNTS registered=%d bound=%d live=%d\n", registered, bound, live);
            require(registered==expectedRebound && bound==expectedRebound, "expected rebound source45 actor count");
        ...
        if (phase == 0) {
            ...
            CI_LOOP(it) {
                ...
                if (e->mGenerator && pc_randomizer_p2_bound_source(e->mGenerator)==45) { if (!target) target = e; }
                else ++others;
            }
            ...
            if (checkpoint && !std::strcmp(checkpoint, "1")) {
                ...
                core->exitDayEnd();
                core->exitStage();
                require(!snowRegistered(target), "old checkpoint registry cleared");
        ...
        if (phase == 1) {
            if (frames%60==0 && target) { std::printf("P2_SNOW_NATURAL_HEALTH frame=%u health=%.3f alive=%d\n",frames,target->mHealth,int(target->isAlive())); std::fflush(stdout); }
        ...
        if (phase >= 3) {
            ...
            if (pc_randomizer_checked(TARGET)) {
                ...
                core->exitDayEnd();
                core->exitStage();
                require(!snowRegistered(target), "old registry cleared at stage exit");
                target=nullptr; corpse=nullptr; reloading=true;
                gameflow.mNextOnePlayerSectionID=ONEPLAYER_NewPikiGame;
                gsys->softReset();
...
int main(int argc, char** argv)
{
    ...
    if (!pc_window_init("Snow natural generated route", 960, 540)) return 3;
    pc_settings_init();
