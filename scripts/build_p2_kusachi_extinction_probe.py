#!/usr/bin/env python3
"""Leased private build + replacement-main link for #793 (root worktree copy).
...
then links tools/p2_kusachi_extinction_probe_fixture.cpp against the
pikmin_pc object graph (all objects except pc_main) using the build.ninja
link edge. Prints executable SHA-256 hashes.

Usage:
  py -3.12 scripts/build_p2_kusachi_extinction_probe.py --native <wt> --build <dir>
"""
import argparse
import hashlib
import os
...
def main():
    ...
    obj_dir = os.path.join(bld, "probe-fixture")
    os.makedirs(obj_dir, exist_ok=True)
    fixture_src = os.path.join(src, "tools",
                               "p2_kusachi_extinction_probe_fixture.cpp")
    fixture_obj = os.path.join(obj_dir,
                               "p2_kusachi_extinction_probe_fixture.cpp.obj")
    ...
    out_exe = os.path.join(obj_dir, "p2_kusachi_extinction_probe_fixture.exe")
