...
// campaign stage, kill an actual enemy, then drive its real corpse through the
// Onion absorption endpoint (GoalItem::suckMe -> pc_randomizer_corpse_delivered
// -> pc_randomizer_check -> checks.txt). Natural Pikmin carry is attempted first;
// a labeled fallback calls the same endpoint directly if the carry stalls.
        ...
        if (phase >= 3) {
            ...
            if (phase == 3 && waited % 60 == 0) {
                ...
                std::printf("P2_ORD_CARRY frame=%u moved=%.2f natural=%d checked=%d\n", frames, moved,
                            int(natural), int(pc_randomizer_checked("Bestiary: Deliver Dwarf Bulborb")));
                std::fflush(stdout);
            }
            ...
            if (phase == 4 && waited > 1500) {
                const bool checked = pc_randomizer_checked("Bestiary: Deliver Dwarf Bulborb");
