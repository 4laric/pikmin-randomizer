"""Lane 06 ordinary Onion/AP receipt runtime acceptance (#441).

Stages a real native randomizer session (``randomizer.seed.generate`` +
``NativeRun`` bootstrap and ``state.txt`` refresh; no live AP server), kills a
real Dwarf Bulborb, drives its real corpse through the real Onion endpoint
(``GoalItem::suckMe`` -> ``pc_randomizer_corpse_delivered`` ->
``pc_randomizer_check``), then re-runs a fresh process against the same session
to prove the ordinary check is granted exactly once across restart.
