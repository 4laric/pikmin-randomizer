...
with tempfile.TemporaryDirectory() as folder:
    ...
    for seed in range(150):
        options=dict(per_spawn_enemies=True,group_spawn_enemies=groups,enemy_shuffle=True,starting_area=(0,1,3,4,5)[seed%5],starting_color=(seed//5)%3,
    ...
    for seed in range(10):
        mw=setup_multiworld([mod.PikminRandomizerWorld]*2,seed=seed+4300,
                           options=[dict(per_spawn_enemies=True,group_spawn_enemies=groups,starting_area=4,starting_color=0),dict(per_spawn_enemies=True,group_spawn_enemies=groups)])
