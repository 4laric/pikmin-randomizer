...
with tempfile.TemporaryDirectory() as folder:
    ...
    for seed in range(150):
        options=dict(per_spawn_enemies=True,enemy_shuffle=True,starting_area=(0,1,3,4,5)[seed%5],starting_color=(seed//5)%3,
    ...
    for seed in range(10):
        mw=setup_multiworld([mod.PikminRandomizerWorld]*2,seed=seed+4300,
                           options=[dict(per_spawn_enemies=True,starting_area=4,starting_color=0),dict(per_spawn_enemies=True)])
        blue=next(item for item in mw.itempool if item.player==1 and item.name=='Blue Onion')
        mw.itempool.remove(blue)
        mw.get_location('Population: 20 total Red Pikmin',2).place_locked_item(blue)
        complete(mw)
print('PASS 150 per-spawn AP fills across all starts/colors and 10 remote-Blue multiworlds; every check reachable')
