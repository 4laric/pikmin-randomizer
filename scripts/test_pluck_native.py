"""Run the real-game pluck fixture in an isolated session."""
import argparse
import os
...
for case in ('pluck',):
    m = generate('pluck-regression', 'ap', collection_checks=True, starting_flarlic=1)
    ...
    with log.open('w', encoding='utf-8') as stream:
        ...
        try:
            ...
            assert p.poll() == 0 and 'PASS pluck' in text, str(log)
            print(case, next(l for l in text.splitlines() if l.startswith('PASS pluck')), flush=True)
        finally:
