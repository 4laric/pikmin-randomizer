...
from randomizer.benefits import BENEFIT_ITEMS, DELIVERY, FLOWERS, HEAL, WHISTLE, PLUCK, BOMBS
...
with tempfile.TemporaryDirectory() as d:
    m = generate('benefits', 'ap', permanent_checks=True, progressive_color_stats=True, bomb_rock_weight=1)
    ...
    ids = [ITEM_IDS[n] for n in BENEFIT_ITEMS if n != HEAL] + [ITEM_IDS[BOMBS]] + [ITEM_IDS[WHISTLE], ITEM_IDS[PLUCK]]
    ...
    run('retracted benefit receipt', lambda text: text.replace('BENEFITS 2 1 0 2 2 1', 'BENEFITS 2 1 0 2 2 0'))
    run('retracted benefit receipt', lambda text: text.replace('BENEFITS 2 1 0 2 2 1', 'BENEFITS 2 1 0 2 2 999'))
    run('missing benefit state', lambda text: text.replace('BENEFITS 2 1 0 2 2 1 ', ''))
