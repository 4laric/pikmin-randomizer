"""Run the real-game bomb selection fixture in an isolated session."""
import argparse
import os
...
for case in ('bomb-selection',):
    m = generate('bomb selection-regression', 'ap', collection_checks=True, starting_flarlic=1)
    ...
    with log.open('w', encoding='utf-8') as stream:
        ...
        try:
            ...
            assert p.poll() == 0 and 'PASS bomb selection' in text, str(log)
            print(case, next(l for l in text.splitlines() if l.startswith('PASS bomb selection')), flush=True)
        finally:
