"""Packaged AP Progg trap classification, YAML option and restrictive fills."""
...
with tempfile.TemporaryDirectory() as folder:
    ...
    for seed in range(30):
        options=dict(progg_trap_weight=(1,5,10)[seed%3],bomb_trap_weight=seed%2,bomb_rock_weight=seed%2,campaign_enemies=True,permanent_checks=True,progressive_color_stats=True,starting_area=(0,1,3,4,5)[seed%5],starting_color=seed%3)
        mw=setup_multiworld(mod.PikminRandomizerWorld,seed=seed,options=options)
        traps=[item for item in mw.itempool if item.name=='Smoky Progg Ambush']
        assert traps and all(item.classification==ItemClassification.trap for item in traps)
        assert mw.worlds[1].manifest()['progg_trap_weight']==options['progg_trap_weight']
    ...
    assert not any(item.name=='Smoky Progg Ambush' for item in mw.itempool)
print('PASS 30 packaged AP Progg trap fills across five areas; trap classification and default-disabled pool')
