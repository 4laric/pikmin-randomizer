"""Build + run helper for the rover stage-table fixture (#734).

Compiles the engine-free provider and its guarded fixture with the system
MinGW g++ (never CMake, never the maintained build), runs the fixture,
validates the P2_ROVER_STAGE_* markers, and records executable/log hashes.
Refuses loudly on any failure. Usage (from the lane root worktree)::

    py -3.12 scripts/build_p2_rover_stage_table.py --native <native-worktree> --output <new-private-dir>
...
def main(argv=None):
    ...
    provider = native / "pc_port" / "pc_p2_challenge_rover_stage.cpp"
    fixture = native / "tools" / "p2_rover_stage_table_fixture.cpp"
    for path in (provider, fixture):
        if not path.is_file():
            parser.exit(1, "missing source: %s\n" % path)
    header = native / "pc_port" / "pc_p2_challenge_rover_stage.h"
    ...
    exe = output / "p2_rover_stage_table_fixture.exe"
    ...
    if "P2_ROVER_STAGE_RESOLVED cave=ch_MAT_route_rover ui=27 floors=1" not in text:
        parser.exit(1, "RESOLVED marker missing\n")
    if "P2_ROVER_STAGE_REFUSED reason=unknown-key" not in text:
    ...
    (output / "provenance.json").write_text(json.dumps(record, indent=2) + "\n",
                                            encoding="utf-8")
    print(json.dumps({"status": "built", "checks": "26/26",
