'''Private runtime-acceptance driver for P1 Challenge Navel (chal2, issue 563).
...
verdicts. The #649 input package validate_run_log is called for the
...
FIXTURE_HEAD = 'c549997e7bdf66fb09c0f0756c56e65fdad55861'
GUARD_SHA256 = 'd2f678c9eda75e151eb534077dff9e30ad36ae4796881d971bbd09945f3c3474'
GUARD_NAME = 'p2_fixture_captain_guard.h'
LEVEL = 2
STAGE_ID = 'challenge-2'
STAGE_INDEX = 18
STAGE_FILE = 'stages/chal2.ini'
GEN_FILE = 'stages/chal2/default.gen'
SLOT = 'chal2'
...
def gate_verdicts(facts):
    ...
    if facts['exit_code'] != 0 or facts['layout'] is None:
        return {g: ('UNTESTED', 'no clean successful boot observed') for g in GATES}
    layout_ok = (facts['layout']['id'] == STAGE_ID and facts['layout']['stage_index'] == STAGE_INDEX and facts['layout']['file'] == STAGE_FILE)
    boot_ok = (facts['boot'] is not None and facts['boot']['slot'] == SLOT)
    ...
    if layout_ok and boot_ok and spawn_ok and facts['pass_marker']:
        gates = {'identity_spawn': ('PASS', 'challenge-2 stage 18 chal2.ini with spawns and squad observed uninterrupted')}
    else:
...
def workflow_cli(payload, command):
    cli = CANONICAL_ROOT / 'scripts' / 'pikmin2_workflow.py'
    tmp = Path(os.environ.get('TEMP', str(CANONICAL_ROOT / 'output'))) / ('navel-lease-%d.json' % time.time_ns())
    tmp.write_text(json.dumps(payload), encoding='utf-8')
    try:
        proc = subprocess.run([sys.executable, str(cli), '--root', str(CANONICAL_ROOT), command, '--request', str(tmp)],
        ...
        try:
            ...
            pass
    if proc.returncode:
        raise RuntimeError('Lease CLI %s failed: %s' % (command, proc.stdout[-2000:]))
    return json.loads(proc.stdout)


def acquire_build_lease(key, generation, resource, pid, ttl=3600, timeout=600):
    stop = time.monotonic() + timeout
    while True:
        result = workflow_cli({'key': key, 'generation': generation, 'resource': resource,
                               'pid': pid, 'ttl': ttl}, 'acquire')
        if result.get('acquired'):
            return result['lease']
        if time.monotonic() >= stop:
            raise RuntimeError('Build lease not acquired: %s' % json.dumps(result)[:500])
        time.sleep(10)


def release_build_lease(key, generation, resource, token):
    try:
        workflow_cli({'key': key, 'generation': generation, 'resource': resource,
                      'token': token}, 'release')
    except RuntimeError as exc:
        print('lease release warning: %s' % exc)
...
def phase_build(native, build_dir, output, head, log_path):
    ...
    env = dict(os.environ, PATH=MINGW_BIN + ';' + os.environ.get('PATH', ' '))
    out = output
    out.mkdir(parents=True, exist_ok=True)
    proc = subprocess.run(['cmake', '-S', str(native), '-B', str(build_dir), '-G', 'Ninja',
        '-DCMAKE_MAKE_PROGRAM=' + toolchain_ninja() + '/ninja.exe',
        '-DCMAKE_BUILD_TYPE=RelWithDebInfo',
        '-DCMAKE_C_COMPILER=' + MINGW_BIN + '/gcc.exe', '-DCMAKE_CXX_COMPILER=' + MINGW_BIN + '/g++.exe',
...
def phase_run(exe, assets, run_parent, deadline=300):
    import _winapi
    ...
    with (run / 'native.log').open('w', encoding='utf-8') as log:
        proc = subprocess.Popen([str(Path(exe).resolve()), '--experimental-challenge-level', str(LEVEL)], cwd=run, stdout=log, stderr=subprocess.STDOUT, env=dict(os.environ, PATH=MINGW_BIN + ';' + os.environ.get('PATH', '')))
        try:
            stop = time.monotonic() + deadline
            observed = False
            while proc.poll() is None and time.monotonic() < stop:
                text = (run / 'native.log').read_text(encoding='utf-8', errors='replace')
                if 'CHALLENGE_LAYOUT_READY id=challenge-2' in text and 'stages/chal2/default.gen' in text and text.count('[PC Port] FPS:') >= 1:
                    observed = True
                    break
                time.sleep(0.2)
            text = (run / 'native.log').read_text(encoding='utf-8', errors='replace')
            ...
            if code is None:
                polled = proc.poll()
                code = polled if polled is not None else (0 if observed else 1)
        finally:
...
def phase_guard_negative(guard_dir, out):
    guard = guard_record(guard_dir)
    src = (Path(out) / 'guard-negative.cpp')
    src.write_text('#include "%s"%sint main()%s{%s    p2_fixture_require_captain(true, false, 0.0f, 7);%s    return 0;%s}%s'
                   % (GUARD_NAME, chr(10), chr(10), chr(10), chr(10), chr(10), chr(10)), encoding='utf-8')
    exe = Path(out) / 'guard-negative.exe'
    env = dict(os.environ, PATH=MINGW_BIN + ';' + os.environ.get('PATH', ''))
    proc = subprocess.run([MINGW_BIN + '/g++.exe', '-std=c++17', '-I', str(Path(guard['path']).parent),
                           str(src), '-o', str(exe)],
                          stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True,
                          encoding='utf-8', errors='replace', timeout=300, env=env)
    if proc.returncode or not exe.is_file():
        raise RuntimeError('Negative guard TU failed to compile: ' + proc.stdout[-2000:])
    proc = subprocess.run([str(exe)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True,
                          encoding='utf-8', errors='replace', timeout=120, env=env)
    log = Path(out) / 'guard-negative.log'
    log.write_text('exit=%d%s%s' % (proc.returncode, chr(10), proc.stdout), encoding='utf-8')
    if proc.returncode != 86 or 'P2_FIXTURE_CAPTAIN_DOWN' not in proc.stdout or 'PASS' in proc.stdout:
        raise RuntimeError('Negative guard path did not trip cleanly')
    return guard


def resolve_assets(explicit):
    if explicit is not None:
        assets = Path(explicit)
    else:
        junction = (CANONICAL_ROOT / 'output/workflow/autofill/planning-shards/p1-challenge/prepared'
                    / 'p1-challenge-guarded-runtime-output/boot-chal2'
                    / '2e4fadebb71241fe98942062f052f76e' / 'assets')
        assets = junction.resolve()
    ini = Path(assets) / 'dataDir/stages/chal2.ini'
    gen = Path(assets) / 'dataDir/stages/chal2/default.gen'
    if not ini.is_file() or not gen.is_file():
        raise SystemExit('Assets missing chal2 stage files: ' + str(assets))
    return assets


def write_run_info(run_dir, assets, out):
    chal2 = Path(assets) / 'dataDir/stages/chal2.ini'
    gen = Path(assets) / 'dataDir/stages/chal2/default.gen'
    info = {'run_dir': str(run_dir), 'fresh_uuid': Path(run_dir).name,
            'assets': str(Path(assets).resolve()),
            'chal2_ini': sha256(chal2) if chal2.is_file() else None,
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='P1 Challenge Navel runtime acceptance driver')
    ...
    parser.add_argument('--lane-key', default='p1-challenge-navel-runtime-acceptance')
    parser.add_argument('--lane-generation', type=int, default=2)
    parser.add_argument('--pid', type=int, default=None)
    parser.add_argument('phases', nargs='+', choices=('build', 'run', 'observe', 'guard', 'all'))
    args = parser.parse_args(argv)
    out = args.output
    out.mkdir(parents=True, exist_ok=True)
    guard_record(args.guard_dir)
    order = ('build', 'run', 'observe', 'guard') if 'all' in args.phases else args.phases
    exe = args.exe
    run_dir = None
    code = 0
    lease = None
    resource = 'build:' + str(Path(args.build).resolve().relative_to(CANONICAL_ROOT))
    try:
        if 'build' in order or 'run' in order:
            ...
            if head != FIXTURE_HEAD:
                raise SystemExit('Native head drift: refusing non-pinned fixture source')
            lease = acquire_build_lease(args.lane_key, args.lane_generation, resource,
                                        args.pid or os.getpid())
            (out / 'lease.json').write_text(json.dumps({'resource': resource, 'token': lease['token']}), encoding='utf-8')
        for phase in order:
            if phase == 'build':
                ...
                if exe is None:
                    ...
                    exe = cands[-1]
                code, run_dir = phase_run(exe, resolve_assets(args.assets), out / 'runs', args.deadline)
                ...
                write_run_info(run_dir, resolve_assets(args.assets), out)
                print(json.dumps({'gates': {g: v[0] for g, v in gates.items()}, 'exit_code': code}))
            elif phase == 'guard':
                phase_guard_negative(args.guard_dir, out)
            if code:
                raise SystemExit('Phase failed')
    finally:
        pass  # caller releases the lease after driver exit (self-release is rejected while alive)
    return 0
