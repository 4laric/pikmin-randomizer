#!/usr/bin/env python3
"""Private leased build/run helper for lane kusachi-content-engine-wiring-native (#728).

Exclusive private build dir under output/; canonical lease CLI; live elastic
cap (no hardcoded slots). Builds pikmin_pc (default config), compiles the
content module + fixture TUs with the reference flags (no CMake membership:
serialized after #725 releases CMakeLists), links the guarded fixture as a
replacement main against the private pikmin_pc graph, runs its guard
self-test / negative test and a bounded headed run from a staged asset root,
then records all hashes. Never touches native/build-randomizer, the
maintained checkout, or CMakeLists. No ADMIT; the run adopts the #632 guard
inside the fixture.
"""
import hashlib
import json
...
LANE_DIR = os.path.join(ROOT, "output", "workflow", "autofill",
                        "prerequisites", "kusachi-content-engine-wiring-native")
NATIVE_DIR = os.path.join(
    ROOT, "output", "workflow", "autofill", "prerequisites",
    "kusachi-content-engine-wiring-native-native")
BUILD_DIR = os.path.join(ROOT, "output", "kusachi-content-engine-wiring-build")
OUT_DIR = os.path.join(LANE_DIR, "out")
RESOURCE = "build:output/kusachi-content-engine-wiring-build"
...
NINJA = os.path.join(os.environ.get("LOCALAPPDATA", ""), "Packages",
                     ...
                     "Scripts", "ninja.exe")
KEY, GEN = "kusachi-content-engine-wiring-native", 2
ISSUE = 728
...
def compile_tu(log, ref_cmd, ref_file, src_name, obj_path, env):
    src = os.path.join(NATIVE_DIR, "pc_port" if "challenge_content" in src_name
                       else "tools", src_name)
    cmd = ref_cmd.replace(ref_file.replace("/", "\\"), src)
    cmd, n = re.subn(r"-o\s+\S+", lambda m: "-o " + obj_path, cmd, count=1)
    assert n == 1, src_name
...
def main():
    ...
    exe = os.path.join(BUILD_DIR, "p2_kusachi_content_wiring.exe")
    try:
        ...
        content_obj = os.path.join(BUILD_DIR, "p2_kusachi_content.obj")
        code = compile_tu(log, ref_cmd, ref_file,
                          "pc_p2_challenge_content.cpp", content_obj, env)
        record["steps"]["content_compile"] = {"exit": code}
        save()
        if code != 0:
            return code
        fixture_src = os.path.join(NATIVE_DIR, "tools",
                                   "p2_kusachi_content_wiring_fixture.cpp")
        fixture_obj = os.path.join(BUILD_DIR, "p2_kusachi_content_wiring_fixture.obj")
        cmd = ref_cmd.replace(ref_file.replace("/", "\\"), fixture_src)
        cmd, n = re.subn(r"-o\s+\S+", lambda m: "-o " + fixture_obj, cmd, count=1)
        assert n == 1
        code, _ = run_logged(log, ["cmd", "/c", cmd], cwd=BUILD_DIR, env=env)
        ...
        for i, o in enumerate(objects):
            if o.replace("\\", "/").endswith("pc_port/pc_main.cpp.obj"):
                ...
                swapped = True
        assert swapped
        # The content module has no CMake membership yet (serialized after
        # #725); link its object explicitly alongside the fixture.
        objects.append(content_obj)
        rsp = os.path.join(BUILD_DIR, "p2_kusachi_content_wiring.rsp")
        ...
        if not record["steps"]["negative_test"]["verified"]:
            return 1
        # Headed run from the staged asset root so the boot reaches idle and
        # the content module can bind the kusachi roster.
        run_env = dict(env)
        run_env["SDL_AUDIODRIVER"] = "dummy"
        run_env["PIKMIN_RANDOMIZER_TEST_BACKGROUND"] = "1"
        markers = os.path.join(OUT_DIR, "wiring-run.log")
        ...
        for line in out.splitlines():
            if "P2_CHALLENGE_CONTENT_WIRED" in line:
                wired = line.strip()
        record["steps"]["wiring_run"] = {
            "exit": rcode, "wired_line": wired,
