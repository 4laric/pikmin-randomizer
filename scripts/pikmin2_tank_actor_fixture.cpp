...
class TankActorFixture:public PlugPikiApp {
 int frames=0,ready=0,moving=0,resetFrame=0,waterFrame=0;Teki* actor=nullptr;Teki* control=nullptr;Vector3f origin;float farthest=0;bool resetDone=false,finish=false;
public:
 int idle() override {
  int result=PlugPikiApp::idle();require(++frames<2400,"Tank actor timeout");
  ...
  if(ready==1){
   ...
   Vector3f near=origin+Vector3f(0,0,400);near.y=mapMgr->getMinY(near.x,near.z,true);n->resetPosition(near);
  }
  ...
  if(!waterFrame&&ready>=300&&farthest>15&&moving>=15){capture("tank-actor-moved.ppm");Vector3f near(300,0,2100);near.y=mapMgr->getMinY(near.x,near.z,true);n->resetPosition(near);waterFrame=ready;}
  if(waterFrame&&!resetDone&&ready>=waterFrame+60){capture("tank-water-display.ppm");pc_p2_tank_reset();resetDone=true;resetFrame=ready;std::puts("P2_TANK_ARENA_RESET");}
 ...
 void draw(Graphics& gfx) override {
  ...
  if(control){Matrix4f unused;require(!pc_p2_tank_draw(control,gfx,unused),"ordinary control acquired active proxy visuals");}
