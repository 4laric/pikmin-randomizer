...
for bombs in (0,1):
    for combined in (False,True):
        with tempfile.TemporaryDirectory() as d:
            m=generate('ambush','ap',permanent_checks=True,bomb_trap_weight=1,bomb_rock_weight=bombs,combined_captain=combined)
            s=Session(m,d);s.bind_ap('room',0,1)
            def run(expected,save=False,corrupt=False):
                r=NativeRun(s);r.write_state(True)
                assert f'BENEFITS {5+bombs+2*combined}\n' in r.bootstrap.read_text()
                if corrupt:
                    state=r.directory/'state.txt'
                    atomic_write(state,state.read_text().replace('BENEFITS 0 0 0 0 0 0 3','BENEFITS 0 0 0 0 0 0 1'))
            ...
            run(0)
            s.receive(0,[ITEM_IDS[TRAP]]*2);s.receive(0,[ITEM_IDS[TRAP]]*2)
            run(2) # unsaved effects replay on restart, like other consumables
            run(2,save=True)
            s=Session(m,d);run(0)
            s.receive(2,[ITEM_IDS[TRAP]])
            run(1,save=True);run(0);run(0,corrupt=True)
print('PASS trap duplicate receipts, modes 5-8, unsaved rollback, saved consumption, reload, retraction rejection')
