...
class Session:
    def __init__(self, manifest, directory):
        ...
        for journal in self.directory.glob("runs/*/checks.txt"):
            ...
            if len(fields) != ((23 if manifest['schema'] >= 9 else 21 if manifest['schema'] >= 6 else 19 if manifest['schema'] >= 4 else 17) + (2 if 'starting_flarlic' in manifest else 0) + (16 if 'color_stats' in manifest else 0) + (2 if manifest.get('progressive_color_stats') else 0) + (2 if manifest.get('benefit_items') else 0) + (33 if 'spawn_layout' in manifest else 0) + (27 if 'group_layout' in manifest else 0)) or fields[:2] != ["PIKMIN_RANDOMIZER", str(manifest["schema"])] or fields[2:4] != ["SESSION", journal.parent.name] or fields[4:6] != ["FINGERPRINT", self.fingerprint]:
