...
class RuntimeEvidenceTests(unittest.TestCase):
 def sample(self):
  ...
  text+='P2_FROG_CLEANUP registered_before=4 cleared=4 reentry=4\n'
  return text
 def test_complete_and_incomplete(self):
  ...
  self.assertFalse(validate(text.replace('P2_FROG_CLEANUP registered_before=4 cleared=4 reentry=4\n',''),0)['passed'])
