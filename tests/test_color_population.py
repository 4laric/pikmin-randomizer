...
class ColorPopulationTests(unittest.TestCase):
    def test_catalog_and_old_seed(self):
        ...
        for permanent in (False, True):
            ...
            old = copy.deepcopy(m)
            old.pop('benefit_items'); old.pop('repair_pool_count'); old['capabilities'].remove('benefit-items-v1')
            old.pop('no_sticks'); old.pop('compact_population'); old['capabilities'].remove('compact-population-v1')
