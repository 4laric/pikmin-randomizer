"""Focused tests for experimental/pikmin2_yakushima_p1_runtime.py.
...
from experimental.pikmin2_yakushima_p1_runtime import (
    stage_run_layout,
    validate_run_layout,
    verify_run_log,
)
...
P1_RUN = os.path.join(
    ...
    "overworld-yakushima/prepared/yakushima-p1-native-runtime/out/p1-run")

MANIFEST = os.path.join(P1_RUN, "manifest.json")
SEED = os.path.join(P1_RUN, "session-seed.json")
PINS = {"root": "c20a22ec", "native": "58d18dc9"}


class YakushimaP1RuntimeTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.mkdtemp(prefix="yakushima-p1-")
    ...
    def test_stage_happy(self):
        ...
        self.assertEqual(record["schema"], "p2-yakushima-p1-run-1")
        self.assertEqual(record["source_pins"], PINS)
        self.assertEqual(validate_run_layout(outdir), [])
        self.assertTrue(os.path.isfile(os.path.join(outdir, "yakushima-p1", "manifest.json")))
    ...
    def test_stage_rejects_bad_pins_json(self):
        from experimental.pikmin2_yakushima_p1_runtime import main
    ...
    def test_validate_detects_tamper(self):
        ...
        with open(os.path.join(outdir, "yakushima-p1", "manifest.json"), "a",
                  encoding="utf-8") as f:
            f.write("# tampered\n")
    ...
    def test_validate_rejects_bad_schema(self):
        outdir, _record = self.staged()
        meta = os.path.join(outdir, "yakushima-p1", "run-metadata.json")
    ...
    def test_verify_full_pass(self):
        log = ("P2_YAKUSHIMA_P1_WINDOW size=960x540\n"
               "P2_YAKUSHIMA_P1_ENGINE_FACT observed=120 squad_alive=20 diagnostic_only=1\n"
               "P2_YAKUSHIMA_P1_BOOT_PASS\nP2_YAKUSHIMA_P1_DAY_PASS\nP2_YAKUSHIMA_P1_SAVE_PASS\n"
               "P2_YAKUSHIMA_P1_RECEIPT_PASS\nP2_YAKUSHIMA_P1_REENTRY_PASS\n"
               "PASS YAKUSHIMA_P1_RUNTIME\n")
    ...
    def test_verify_unsupported_boundaries(self):
        log = ("P2_YAKUSHIMA_P1_WINDOW size=960x540\n"
               "P2_YAKUSHIMA_P1_ENGINE_FACT observed=120 squad_alive=20 diagnostic_only=1\n"
               "P2_YAKUSHIMA_P1_UNSUPPORTED boundary=day_transition reason=no-day-advance-api-in-port\n"
               "FAIL YAKUSHIMA_P1_RUNTIME boundaries_unobservable=5 observed=120\n")
        result = verify_run_log(log)
        self.assertTrue(result["engine_boot"])
        self.assertFalse(result["overall_pass"])
        self.assertEqual(result["boundaries"]["day_transition"], "unsupported")
        self.assertEqual(result["boundaries"]["boot_yakushima_surface"], "unobserved")
        self.assertIn("FAIL YAKUSHIMA_P1_RUNTIME", result["failure_markers"])

    def test_verify_captain_down_blocks_pass(self):
        log = ("P2_YAKUSHIMA_P1_WINDOW size=960x540\n"
               "P2_YAKUSHIMA_P1_ENGINE_FACT observed=5 squad_alive=20 diagnostic_only=1\n"
               "P2_YAKUSHIMA_P1_BOOT_PASS\nP2_YAKUSHIMA_P1_DAY_PASS\nP2_YAKUSHIMA_P1_SAVE_PASS\n"
               "P2_YAKUSHIMA_P1_RECEIPT_PASS\nP2_YAKUSHIMA_P1_REENTRY_PASS\n"
