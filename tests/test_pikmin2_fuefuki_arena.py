...
def test_gate_status_review_corrected():
    assert STATUS['brain_fallback'].startswith('pass (policy)')
    assert STATUS['press_combat'].startswith('untested')
    assert 'provider lane 10' in STATUS['press_combat']
