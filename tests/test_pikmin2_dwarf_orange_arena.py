...
import experimental.pikmin2_dwarf_orange_arena as arena
...
def test_family_actor_plus_control_unique_ids(tmp_path):
    ...
    assert [a['species'] for a in actors] == ['BlueKochappy', 'P1 Chappy']
    assert [a['generator'] for a in actors] == [211001, 211002]
...
def test_invalid_expected_xyz_rejected(tmp_path, monkeypatch):
    monkeypatch.setattr(arena, 'POSITIONS', ((float('nan'), 30., 1850.), (150., 30., 1550.)))
