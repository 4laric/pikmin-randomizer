"""Focused P0 tests for the ch_NARI_02tile import adapter (issue #537).
...
ADAPTER_PATH = (
    ...
    / "p2-challenge-ch_nari_02tile.py"
)
_spec = importlib.util.spec_from_file_location("ch_nari_02tile_adapter", ADAPTER_PATH)
...
SOURCE_FILE = Path(
    "C:/Users/alari/pikmin-randomizer/output/workflow/content-expansion"
    "/p2-challenge-ch_nari_02tile/ch_NARI_02tile.txt"
)
HAS_SOURCE = SOURCE_FILE.is_file()

EXPECTED_ENEMIES_F1 = ["UmiMushiBlind_key", "Jigumo_toy_ring_a_blue",
                       "Jigumo_toy_ring_b_green", "Demon_diamond_blue_l",
                       "OniKurage_diamond_red"]
EXPECTED_TREASURES_F1 = ["ahiru", "toy_dog", "bell", "diamond_red_l", "diamond_green_l"]
EXPECTED_ENEMIES_F2 = ["UmiMushi_key"]
EXPECTED_TREASURES_F2 = ["ahiru_head", "diamond_blue", "toy_ring_a_red"]
EXPECTED_POOLS = ["2_units_northF_pool_tile.txt", "1_MIYA_bunki_tile.txt"]
...
class StructuralScanTests(unittest.TestCase):
    ...
    def test_observed_rosters_match_source(self):
        ...
        self.assertEqual([c["token"] for c in first["caps"] if not c["empty"]], ["Egg"])
        self.assertEqual([c["token"] for c in second["caps"] if not c["empty"]], ["WhitePom"])
        self.assertEqual(first["enemies"][0]["source_weight"], 10)
        self.assertEqual(first["enemies"][3]["placement_type"], 6)
    ...
    def test_real_source_mutations_fail_closed(self):
        ...
        for text in (good + " trailing", good[:good.rfind("}")],
                     good.replace("{f008} -1 2_units_northF_pool_tile.txt",
                                  "{f008} -1 ../evil.txt")):
            with self.subTest(text=text[-40:]), self.assertRaises((ValueError, UnsupportedDefinition)):
                structural_scan(text)
