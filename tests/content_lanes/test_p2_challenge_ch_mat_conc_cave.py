"""Focused contract tests for the ch_MAT_conc_cave P0 adapter (issue #536).

Positive pins come from the catalogued baseline (observed values, never
invented): 3 floors, timers [70.0, 100.0, 50.0], starting-population total
2 at cell [4][2], sprays 0/0, treasure field 0, ui_index 2, table_order 11,
sha pin b3ae2c41... The shared cave parser is exercised on a synthetic
minimal 3-floor caveinfo text (labeled synthetic, never presented as
source evidence). Negative tests cover malformed/missing inputs and the
...
WORKTREE = Path("C:/Users/alari/pikmin-randomizer/output/content-p0-536")
ADAPTER_PATH = WORKTREE / "experimental/content_lanes/p2-challenge-ch_mat_conc_cave.py"


def load_adapter():
    spec = importlib.util.spec_from_file_location("p2_challenge_ch_mat_conc_cave", ADAPTER_PATH)
...
ENTRY = next(s for s in INVENTORY["challenge"]["stages"] if s["cave_id"] == "ch_MAT_conc_cave")
...
def floor_block(first, pool, enemies="0", treasures="0", gates="0"):
    return ("{ {f000} 4 %d {f001} 4 %d {f008} -1 %s {_eof} } "
            "{ %s } { %s } { %s }") % (first, first, pool, enemies, treasures, gates)


SYNTHETIC_3 = ("{ {c000} 4 3 {_eof} } 3 "
               + floor_block(0, "poolA.txt", "1 Pelplant 10 1",
                             "1 %s 5" % TREASURE, "1 gateA 100.0 5")
               + " " + floor_block(1, "poolB.txt")
               + " " + floor_block(2, "poolA.txt"))


class LaneEntryTests(unittest.TestCase):
    def test_finds_lane_entry(self):
        entry = adapter.find_lane_entry(LANES)
        self.assertEqual(entry["lane"], "p2-challenge-ch_mat_conc_cave")
        self.assertEqual(entry["issue"], 536)
        self.assertEqual(entry["source"], "user/Mukki/mapunits/caveinfo/ch_MAT_conc_cave.txt")
    ...
    def test_finds_stage_entry(self):
        ...
        self.assertEqual(stage["cave_id"], "ch_MAT_conc_cave")
        self.assertEqual(stage["floors"], 3)
...
class MetadataAuditTests(unittest.TestCase):
    def test_catalogued_pins(self):
        ...
        self.assertEqual(audit["floors"], 3)
        self.assertEqual(audit["floor_seconds"], [70.0, 100.0, 50.0])
        self.assertEqual(audit["total_starting_pikmin"], 2)
        self.assertEqual(audit["nonzero_population_cells"], [(4, 2)])
        self.assertEqual(audit["bitter_sprays"], 0)
        self.assertEqual(audit["spicy_sprays"], 0)
        self.assertEqual(audit["treasure_count_field"], 0)
        self.assertEqual(audit["legacy_time"], 500.0)
        self.assertEqual(audit["ui_index"], 2)
        self.assertEqual(audit["table_order"], 11)
    ...
    def test_timer_length_mismatch(self):
        bad = dict(ENTRY, floor_seconds=[70.0, 100.0])
        with self.assertRaises(ValueError):
            adapter.audit_metadata(bad)

    def test_nonpositive_timer(self):
        bad = dict(ENTRY, floor_seconds=[70.0, 0.0, 50.0])
        with self.assertRaises(ValueError):
            adapter.audit_metadata(bad)
    ...
    def test_string_ui_index(self):
        with self.assertRaises(ValueError):
            adapter.audit_metadata(dict(ENTRY, ui_index="2"))
...
class DecodeBoundaryTests(unittest.TestCase):
    def test_synthetic_three_floor_decode(self):
        parsed = adapter.decode_source_text(SYNTHETIC_3, ENEMIES, TREASURES)
        summary = adapter.summarize_decoded(parsed)
        self.assertEqual(summary["floor_count"], 3)
        self.assertEqual(summary["unit_pools_ordered"], ["poolA.txt", "poolB.txt", "poolA.txt"])
        self.assertEqual(summary["unit_pools_distinct"], ["poolA.txt", "poolB.txt"])
    ...
    def test_truncated_text_rejected(self):
        with self.assertRaises(ValueError):
            adapter.decode_source_text("{ {c000} 4 3 {_eof} } 3", ENEMIES, TREASURES)

    def test_count_mismatch_rejected(self):
        text = "{ {c000} 4 2 {_eof} } 3 " + floor_block(0, "poolA.txt")
        with self.assertRaises(ValueError):
            adapter.decode_source_text(text, ENEMIES, TREASURES)

    def test_unknown_enemy_rejected(self):
        text = ("{ {c000} 4 1 {_eof} } 1 "
                + floor_block(0, "poolA.txt", "1 NoSuchEnemyXYZ 10 1"))
        with self.assertRaises(ValueError):
            adapter.decode_source_text(text, ENEMIES, TREASURES)

    def test_coverage_mismatch_rejected(self):
        text = "{ {c000} 4 2 {_eof} } 2 " + floor_block(0, "poolA.txt") + " " + floor_block(1, "poolB.txt")
...
class PacketTests(unittest.TestCase):
    def test_packet_shape(self):
        ...
        self.assertEqual(packet["lane"], "p2-challenge-ch_mat_conc_cave")
        self.assertEqual(packet["issue"], 536)
        ...
        self.assertEqual(packet["lane_entry_issue"], 536)
