"""Focused boundary tests for the ch_NARI_09suikomi P0 adapter (#558).
...
ADAPTER = (Path(__file__).resolve().parents[2]
           ...
           / "p2-challenge-ch_nari_09suikomi.py")


def load_adapter():
    spec = importlib.util.spec_from_file_location("suikomi_adapter", ADAPTER)
...
class BaselineContractTests(unittest.TestCase):
    ...
    def test_resource_closure_totals_without_placements(self):
        ...
        self.assertEqual(closure["starting_roster_total"], 60)
        self.assertEqual(closure["starting_roster_by_row"], [0, 30, 30, 0, 0, 0, 0])
        self.assertEqual(closure["starting_roster_by_col"], [0, 0, 60])
        self.assertEqual(closure["floor_seconds"], [180.0])
        self.assertEqual(closure["bitter_sprays"], 1)
        self.assertEqual(closure["spicy_sprays"], 2)
    ...
    def test_packet_is_json_serializable(self):
        ...
        self.assertEqual(
            packet["playability"],
            "none claimed; P1 runtime and P2 acceptance remain OPEN")
...
class MalformedInputTests(unittest.TestCase):
    ...
    def test_non_object_rejected(self):
        for bad in (None, [], "ch_NARI_09suikomi"):
    ...
    def test_missing_and_extra_keys_rejected(self):
        ...
        record = self.altered()
        record["english_title"] = "Suikomi"
        with self.assertRaises(adapter.StageRecordError):
            adapter.validate_stage_record(record)

    def test_identity_mismatch_rejected(self):
        for key, bad in (("cave_id", "ch_MUKI_damagumo"),
                         ("cave_path",
                          "user/Mukki/mapunits/caveinfo/ch_MUKI_damagumo.txt"),
                         ("table_order", 9), ("floors", 2),
                         ("ui_index", 6), ("issue", 569)):
            record = self.altered()
            record[key] = bad
    ...
    def test_roster_shape_and_sign_rejected(self):
        record = self.altered()
        record["pikmin_by_native_color_and_maturity"] = [[0, 0, 60]]
        with self.assertRaises(adapter.StageRecordError):
            adapter.validate_stage_record(record)
        record = self.altered()
        record["pikmin_by_native_color_and_maturity"][1][2] = -1
        with self.assertRaises(adapter.StageRecordError):
            adapter.validate_stage_record(record)
        record = self.altered()
        record["pikmin_by_native_color_and_maturity"][1][2] = True
        with self.assertRaises(adapter.StageRecordError):
            adapter.validate_stage_record(record)

    def test_timer_floor_disagreement_rejected(self):
        record = self.altered()
        record["floor_seconds"] = [180.0, 180.0]
        with self.assertRaises(adapter.StageRecordError):
            adapter.validate_stage_record(record)
        record = self.altered()
...
class SourceBoundaryTests(unittest.TestCase):
    ...
    def test_missing_prerequisites_name_exact_files(self):
        ...
        self.assertIn(
            "user/Mukki/mapunits/caveinfo/ch_NARI_09suikomi.txt", paths)
        self.assertIn("user/Matoba/challenge/stages.txt", paths)
        entry = next(m for m in missing
                     if m["path"].endswith("ch_NARI_09suikomi.txt"))
    ...
    def test_present_but_foreign_file_reported_as_mismatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (target / "ch_NARI_09suikomi.txt").write_bytes(b"not the retail file")
            missing = adapter.missing_prerequisites([tmp])
        entry = next(m for m in missing
                     if m["path"].endswith("ch_NARI_09suikomi.txt"))
