"""Focused P0 contract tests for the ch_MAT_flier adapter (#559).
...
def _load_adapter():
    path = ROOT / "experimental" / "content_lanes" / "p2-challenge-ch_mat_flier.py"
    spec = importlib.util.spec_from_file_location("matflier_adapter", path)
...
def _inputs():
    ...
    plan_entry = next(lane for lane in plan["lanes"]
                      if lane.get("lane") == "p2-challenge-ch_mat_flier")
    ...
    stage = next(row for row in challenge["stages"]
                 if row.get("cave_id") == "ch_MAT_flier")
    return (plan_entry, stage, challenge, inventory["source_sha256"])


class MatFlierContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.args = _inputs()
    ...
    def test_real_sources_validate(self):
        ...
        self.assertEqual(result["details"]["floor_seconds"], [160.0])
        self.assertEqual(result["details"]["ui_index"], 28)
        self.assertEqual(result["details"]["table_order"], 26)
    ...
    def test_ledger_conserves_definitions(self):
        ...
        self.assertEqual(ledger["starting_pikmin_total"], 50)
        self.assertEqual(ledger["starting_populations"],
                         [{"native_color": 1, "maturity": 2, "count": 50}])
        self.assertEqual(ledger["floor_timer_total"], 160.0)
        self.assertEqual((ledger["bitter_sprays"], ledger["spicy_sprays"]), (1, 1))
    ...
    def test_wrong_lane_rejected(self):
        ...
        plan_entry["lane"] = "p2-challenge-ch_nari_06start3hard"
    ...
    def test_population_and_spray_drift_rejected(self):
        ...
        bad_stage["pikmin_by_native_color_and_maturity"][1] = [0, 0, 51]
        with self.assertRaises(ValueError):
    ...
    def test_duplicate_row_and_index_collision_rejected(self):
        ...
        collided["stages"][0]["table_order"] = 26
        with self.assertRaises(ValueError):
    ...
    def test_missing_and_placeholder_hash_rejected(self):
        ...
        hashes.pop("user/Mukki/mapunits/caveinfo/ch_MAT_flier.txt")
        with self.assertRaises(ValueError):
