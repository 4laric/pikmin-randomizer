"""Focused P0 tests for the ch_MUKI_bombing import-contract adapter (#556).
...
_spec = importlib.util.spec_from_file_location(
    'p2_challenge_ch_muki_bombing',
    ROOT / 'experimental/content_lanes/p2-challenge-ch_muki_bombing.py')
...
ENEMIES = {'BombSarai', 'BlueChappy'}
TREASURES = {'be_dama_blue_l'}
...
def test_identity_matches_canonical_inventory_and_plan():
    ...
    entry = next(e for e in inventory['challenge']['stages'] if e['cave_id'] == CAVE_ID)
    ...
    lane = next(e for e in plan['lanes'] if e['lane'] == 'p2-challenge-ch_muki_bombing')
    assert lane['source'] == SOURCE_PATH and lane['source_sha256'] == SOURCE_SHA256
    assert lane['issue'] == 556
...
def test_decode_accepts_single_floor_structure():
    ...
    assert floor['enemies'][0]['enemy_id'] == 'BombSarai'
...
def test_decode_rejects_malformed_inputs():
    bad = ['', '{ {c000} 4 1', '{ {c000} 4 1 {_eof} }\n1\n',
           ONE_FLOOR.replace('BombSarai', 'NotAnEnemy'),
           ONE_FLOOR.replace('{ 1 BombSarai 10 1 }', '{ 2 BombSarai 10 1 }')]
    for text in bad:
        try:
...
def test_packet_is_honest_without_source():
    ...
    assert packet['schema'] == 1 and packet['issue'] == 556
...
def test_write_packet_round_trips(tmp_path):
    ...
    packet = build_import_packet(decode_stage(ONE_FLOOR, ENEMIES, TREASURES), verification)
    record = write_packet(packet, tmp_path / 'p0-556')
