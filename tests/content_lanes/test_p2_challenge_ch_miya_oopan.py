"""Focused P0 tests for the ch_MIYA_oopan import-contract adapter (#554).
...
RESERVED = (Path(__file__).resolve().parents[2] / "experimental" / "content_lanes"
            / "p2-challenge-ch_miya_oopan.py")


def load_lane():
    # Reserved filename contains dashes, so import by path, not dotted name.
    spec = importlib.util.spec_from_file_location("p2_challenge_ch_miya_oopan_lane",
                                                  RESERVED)
...
class LocateTests(unittest.TestCase):
    def test_missing_everywhere_reports_exact_prerequisite(self):
        ...
        self.assertIn("user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt",
                      str(ctx.exception))
    ...
    def test_missing_prerequisite_record(self):
        ...
        self.assertEqual(record["source_id"], "ch_MIYA_oopan")
...
class HashTests(unittest.TestCase):
    ...
    def test_expected_digest_is_inventory_pin(self):
        self.assertEqual(lane.EXPECTED_SHA256,
                         "76fad15ebc041af12f05e887ad64c324354d49fc89acad57d043d9881951b4b9")


class DetailsTests(unittest.TestCase):
    def test_authoritative_details_validate(self):
        record = lane.validate_details()
        self.assertEqual(record["floors"], 1)
        self.assertEqual(record["floor_seconds"], [150.0])
        self.assertEqual(record["bitter_sprays"], 0)
        self.assertEqual(record["spicy_sprays"], 0)
        self.assertEqual(record["treasure_count_field"], 0)
        self.assertEqual(record["ui_index"], 21)
        self.assertEqual(record["table_order"], 15)
        self.assertEqual(len(record["pikmin_by_native_color_and_maturity"]), 7)

    def test_timer_count_mismatch_rejected(self):
        bad = dict(lane.DETAILS, floor_seconds=[150.0, 100.0])
    ...
    def test_roster_shape_rejected(self):
        bad = dict(lane.DETAILS, pikmin_by_native_color_and_maturity=[[5, 0, 0]])
        with self.assertRaises(ValueError):
            lane.validate_details(bad)

    def test_foreign_source_record_rejected(self):
        bad = dict(lane.DETAILS, cave_id="ch_MIYA_trap")
        with self.assertRaises(ValueError):
            lane.validate_details(bad)

    def test_starting_roster_summary(self):
        roster = lane.starting_roster()
        self.assertEqual(
            roster,
            [{"native_color": color, "maturity": 0, "count": 5}
             for color in range(5)])


class DecodeTests(unittest.TestCase):
    def test_synthetic_one_floor_format_decodes(self):
        # Synthetic format fragment only: proves the shared-parser path and
        # floor-coverage logic, claims no retail values.
        floors = lane.decode_definitions(MINIMAL_ONE_FLOOR)
        self.assertEqual(len(floors), 1)
        coverage = lane.floor_coverage(floors, expected=1)
    ...
    def test_unclosed_block_rejected(self):
        with self.assertRaises(ValueError):
            lane.decode_definitions("{ c000 4 1")

    def test_floor_gap_detected(self):
        coverage = lane.floor_coverage([{"number": 1}, {"number": 3}], expected=1)
        self.assertFalse(coverage["complete"])

    def test_duplicate_floor_detected(self):
        coverage = lane.floor_coverage([{"number": 1}, {"number": 1}], expected=1)
        self.assertFalse(coverage["complete"])
...
class AuditTests(unittest.TestCase):
    ...
    def test_audit_preserves_roster_and_timers(self):
        report = lane.audit(exists=lambda candidate: False)
        self.assertEqual(report["details"]["floor_seconds"], [150.0])
        self.assertEqual(report["details"]["bitter_sprays"], 0)
        self.assertEqual(report["details"]["spicy_sprays"], 0)
        self.assertEqual(len(report["starting_roster"]), 5)
