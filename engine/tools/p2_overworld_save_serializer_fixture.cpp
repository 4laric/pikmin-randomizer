// Guarded replacement-main fixture proving overworld save serialization (#736).
//
// Boots the REAL game-linked engine over the P2 room preview, sets explicit
// session context (area yakushima), and requires a verified save/reload
// round-trip of the live session (squad counted live from pikiMgr) plus a
// live squad before PASS. No injection, no simulation: only engine state and
// explicit fixture argv are ever recorded.
//
...
//
// Markers: P2_SAVE_FIXTURE_* only, plus the module's own
// P2_OVERWORLD_SAVE_SAVED / LOADED / SUPPORTED / REFUSED markers. Successful
// boot ends with "PASS SAVE_FIXTURE" and exit 0. Anything else is FAIL (1),
...
bool sForceCaptainDown = false; // env P2_SAVE_FIXTURE_FORCE_CAPTAIN_DOWN=1: negative-path test only
...
int guardSelfTest() {
    ...
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        ...
        if (down != rows[i].expectDown) {
            std::printf("FAIL SAVE_FIXTURE selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
    ...
    std::printf("P2_SAVE_FIXTURE_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}

class SaveFixtureApp final : public PlugPikiApp {
...
public:
    ...
    int idle() override {
        ...
        if (++frames > 30000) {
            std::printf("FAIL SAVE_FIXTURE timeout observed=%d verified=%d\n",
                        observed, int(p2overworldsave::supported()));
            std::fflush(stdout);
            std::_Exit(2);
        }
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_SAVE_FIXTURE_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
        ...
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_SAVE_FIXTURE_WAIT frames=%d navi=0\n", frames);
                std::fflush(stdout);
            }
            return result;
        }
        ...
        ++observed;
        // Direct module proof (never injected): collect the live session and
        // verify a save/reload round-trip to the fixture save file. PASS
        // requires that verified round-trip plus a live squad.
        p2overworldsave::Session session;
        std::memset(&session, 0, sizeof(session));
        const bool collected = p2overworldsave::collectLiveSession(session);
        bool verified = false;
        if (collected)
            verified = p2overworldsave::verifyRoundTrip("p2-overworld-save-fixture.txt", session);
        if (!readyLogged && observed % 600 == 0) {
            std::printf("P2_SAVE_FIXTURE_WAIT observed=%d collected=%d verified=%d supported=%d\n",
                        observed, int(collected), int(verified),
                        int(p2overworldsave::supported()));
            std::fflush(stdout);
        }
        if (observed >= 600 && !readyLogged) {
            readyLogged = true;
            std::printf("P2_SAVE_FIXTURE_READY observed=%d collected=%d verified=%d supported=%d\n",
                        observed, int(collected), int(verified),
                        int(p2overworldsave::supported()));
            std::fflush(stdout);
        }
        if (readyLogged) {
            ...
            if (alive > 0 && verified) {
                std::printf("P2_SAVE_FIXTURE_PASS verified=%d squad_alive=%d observed=%d path=%s\n",
                            int(verified), alive, observed,
                            p2overworldsave::lastPath());
                std::puts("PASS SAVE_FIXTURE");
                std::fflush(stdout);
                std::_Exit(0);
            }
        }
...
int main(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::string(argv[i]) == "--guard-negative-test") {
            ...
            std::printf("FAIL SAVE_FIXTURE negative test did not trip\n");
            std::fflush(stdout);
            return 1;
        }
    }
    if (sGuardSelfTest) return guardSelfTest();
    const char* force = std::getenv("P2_SAVE_FIXTURE_FORCE_CAPTAIN_DOWN");
    ...
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL SAVE_FIXTURE requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    if (!pc_window_init("P2 overworld save serializer fixture", 960, 540)) return 3;
    pc_window_center();
    {
        ...
        std::printf("P2_SAVE_FIXTURE_WINDOW size=%dx%d pos=%d,%d display=%dx%d centered=%d\n",
    ...
    nodeMgr = new NodeMgr();
    if (!p2overworldsave::setContext("yakushima", 1, 0, 0)) {
        std::printf("FAIL SAVE_FIXTURE bad session context\n");
        std::fflush(stdout);
        return 1;
    }
    gsys->run(new SaveFixtureApp());
    return 0;
}
