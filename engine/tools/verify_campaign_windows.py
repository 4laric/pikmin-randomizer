...
parser = argparse.ArgumentParser(description='Build the live campaign_enemies regression fixture against a completed Windows Ninja game build.')
...
compile_args.append('-Wno-error=switch')  # Legacy gameCore headers use partial enum switches.
# Test-only initial day override exercises scheduled Impact/Spring encounters.
setup=output/'campaign_gameSetup.cpp'
setup.write_text((source/'src/plugPikiColin/gameSetup.cpp').read_text(encoding='utf-8').replace('gameflow.mWorldClock.mCurrentDay = 2;', 'gameflow.mWorldClock.mCurrentDay = std::getenv("PIKMIN_ENEMY_TEST_DAY") ? std::atoi(std::getenv("PIKMIN_ENEMY_TEST_DAY")) : 2;'),encoding='utf-8')
setup_args=list(compile_args)
setup_obj=output/'campaign_gameSetup.obj'
setup_args[setup_args.index('-o')+1]=str(setup_obj)
setup_args[setup_args.index('-c')+1]=str(setup)
link_args.insert(link_args.index(str(obj))+1,str(setup_obj))
with (output/'build.log').open('w', encoding='utf-8') as log:
    subprocess.run(compile_args,cwd=build,stdout=log,stderr=subprocess.STDOUT,check=True)
    subprocess.run(setup_args,cwd=build,stdout=log,stderr=subprocess.STDOUT,check=True)
