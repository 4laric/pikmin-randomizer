"""Run and verify the private Waterwraith ACTOR visual runtime fixture (#443).

Mirrors tools/p2_waterwraith_runtime_run.py: verifies fixture provenance, stages
a fresh room overlay via scripts/preview_pikmin2_room.py, copies the two-species
visual profile and pose .mod files in, runs the fixture with
--experimental-pikmin2-room, and checks the actor window/visual/PASS markers.
"""
import argparse
import hashlib
...
MARKERS = ("P2_WATERWRAITH_ACTOR_READY", "P2_WATERWRAITH_ACTOR_DRAW",
           "P2_WATERWRAITH_ACTOR_PLAY", "P2_WATERWRAITH_ACTOR_WINDOW",
           "P2_WATERWRAITH_ACTOR_PASS", "PASS WATERWRAITH_ACTOR_RUNTIME")
...
def verify_log(text):
    ...
    if not re.search(r"^P2_WATERWRAITH_ACTOR_WINDOW\s+size=960x540\s+pos=-?\d+,-?\d+\s*$", text,
                     flags=re.MULTILINE):
        errors.append("missing 960x540 window marker")
    ready = re.search(r"^P2_WATERWRAITH_ACTOR_READY\s+species=(\d+)\s+clips=(\d+)\s*$", text,
                      flags=re.MULTILINE)
    if not ready:
        errors.append("missing actor READY marker")
    elif int(ready.group(1)) != 2:
        errors.append(f"expected 2 species: {ready.groups()}")
    draw = re.search(r"^P2_WATERWRAITH_ACTOR_DRAW\s+species=(\d+)\s*$", text, flags=re.MULTILINE)
    if not draw:
        errors.append("missing actor DRAW marker")
    elif int(draw.group(1)) != 1:
        errors.append(f"expected per-species draw marker: {draw.groups()}")
    passed = re.search(r"^P2_WATERWRAITH_ACTOR_PASS\s+ticks=(\d+)\s+distance=([\d.]+)\s+"
                       r"roll=([\d.]+)\s+phase=(\w+)\s*$", text, flags=re.MULTILINE)
    if not passed:
        errors.append("missing actor PASS marker")
    elif float(passed.group(2)) <= 0.0 or float(passed.group(3)) <= 0.0:
        errors.append(f"roller did not travel/roll: {passed.groups()}")
    return errors
...
def main(argv=None):
    ...
    try:
        ...
        capture = run / "waterwraith-actor.ppm"
        if not capture.is_file():
            errors.append("missing capture: waterwraith-actor.ppm")
        elif capture.stat().st_size < 100000:
            errors.append("trivial capture: waterwraith-actor.ppm")
        else:
            record["captures"] = {"waterwraith-actor.ppm": file_record(capture)}
