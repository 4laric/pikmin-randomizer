// tutorial_2 P1 guarded replacement-main fixture (#152).
//
...
// specialised marker namespace P2_TUTORIAL2_P1_* for the tutorial_2 cave
...
// emitted by experimental/content_lanes/p2-cave-tutorial_2_p1.py. No new
...
// follow-up (see docs/PIKMIN2_TUTORIAL2_P1_FIXTURE.md); the lane build
// script links this TU against the private pikmin_pc graph without editing
// shared build files.
//
// Markers: P2_TUTORIAL2_P1_* only, plus the engine's own P2_CAVE_READY /
// P2_CAVE_GENERATE_* markers. Successful boot ends with
// "PASS TUTORIAL2_P1" and exit 0. Anything else is FAIL (1), BLOCKED
...
bool sForceCaptainDown = false; // env P2_TUTORIAL2_P1_FORCE_CAPTAIN_DOWN=1: negative-path test only
...
int guardSelfTest() {
    ...
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        ...
        if (down != rows[i].expectDown) {
            std::printf("FAIL TUTORIAL2_P1 selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
    ...
    std::printf("P2_TUTORIAL2_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}
...
public:
    int idle() override {
        ...
        if (++frames > 30000) {
            std::printf("FAIL TUTORIAL2_P1 timeout entry_seen=%d observed=%d\n",
        ...
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_TUTORIAL2_P1_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
        ...
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_TUTORIAL2_P1_WAIT frames=%d navi=0 floor=%d\n",
        ...
        if (floor <= 0 && observed % 600 == 0) {
            std::printf("P2_TUTORIAL2_P1_WAIT observed=%d floor=0\n", observed);
            std::fflush(stdout);
        }
        if (floor > 0 && !entrySeen) {
            entrySeen = true;
            std::printf("P2_TUTORIAL2_ENTRY_READY floor=%d observed=%d\n", floor, observed);
            std::fflush(stdout);
        }
        if (entrySeen) {
            ...
            if (alive > 0) {
                ...
                std::printf("P2_TUTORIAL2_P1_PASS floor=%d squad_alive=%d observed=%d\n",
                            floor, alive, observed);
                std::puts("PASS TUTORIAL2_P1");
                std::fflush(stdout);
                std::_Exit(0);
            }
        }
...
int main(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::string(argv[i]) == "--guard-negative-test") {
            ...
            std::printf("FAIL TUTORIAL2_P1 negative test did not trip\n");
            std::fflush(stdout);
            return 1;
        }
    }
    if (sGuardSelfTest) return guardSelfTest();
    const char* force = std::getenv("P2_TUTORIAL2_P1_FORCE_CAPTAIN_DOWN");
    ...
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL TUTORIAL2_P1 requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    ...
    pc_window_center();
    {
        ...
        std::printf("P2_TUTORIAL2_WINDOW size=%dx%d pos=%d,%d display=%dx%d centered=%d\n",
