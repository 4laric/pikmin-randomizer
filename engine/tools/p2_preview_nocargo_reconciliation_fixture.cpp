...
// (env P2_NOCARGO_RECONCILIATION_EXPECT_TREASURE=0/1) under a live squad.
//
...
//
// Markers: P2_NOCARGO_RECONCILIATION_* only, plus the engine's own
// P2_PREVIEW_PR05 reconciliation summary. Successful boot ends with
// "PASS NOCARGO_RECONCILIATION" and exit 0. Anything else is FAIL (1),
...
bool sForceCaptainDown = false; // env P2_NOCARGO_RECONCILIATION_FORCE_CAPTAIN_DOWN=1: negative-path test only
...
int guardSelfTest() {
    ...
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        ...
        if (down != rows[i].expectDown) {
            std::printf("FAIL NOCARGO_RECONCILIATION selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
    ...
    std::printf("P2_NOCARGO_RECONCILIATION_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}
...
public:
    ...
    int idle() override {
        ...
        if (++frames > 30000) {
            std::printf("FAIL NOCARGO_RECONCILIATION timeout observed=%d expect_treasure=%d\n",
        ...
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_NOCARGO_RECONCILIATION_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
        ...
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_NOCARGO_RECONCILIATION_WAIT frames=%d navi=0\n", frames);
                std::fflush(stdout);
            }
            return result;
        }
        ...
        if (!readyLogged && observed % 600 == 0) {
            std::printf("P2_NOCARGO_RECONCILIATION_WAIT observed=%d treasure=%d expect=%d\n",
                        observed, int(hasTreasure), int(expectTreasure));
            std::fflush(stdout);
        }
        if (observed >= 600 && !readyLogged) {
            readyLogged = true;
            std::printf("P2_NOCARGO_RECONCILIATION_READY observed=%d treasure=%d expect=%d\n",
                        observed, int(hasTreasure), int(expectTreasure));
            std::fflush(stdout);
        }
        if (readyLogged) {
            ...
            if (alive > 0) {
                if (hasTreasure != expectTreasure) {
                    std::printf("FAIL NOCARGO_RECONCILIATION treasure=%d expect=%d squad_alive=%d observed=%d\n",
                                int(hasTreasure), int(expectTreasure), alive, observed);
                    std::fflush(stdout);
                    std::_Exit(1);
                }
                std::printf("P2_NOCARGO_RECONCILIATION_PASS treasure=%d squad_alive=%d observed=%d\n",
                            int(hasTreasure), alive, observed);
                std::puts("PASS NOCARGO_RECONCILIATION");
                std::fflush(stdout);
                std::_Exit(0);
            }
        }
...
int main(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        ...
        if (std::string(argv[i]) == "--guard-negative-test") {
            ...
            std::printf("FAIL NOCARGO_RECONCILIATION negative test did not trip\n");
            std::fflush(stdout);
            return 1;
        }
    }
    if (sGuardSelfTest) return guardSelfTest();
    const char* force = std::getenv("P2_NOCARGO_RECONCILIATION_FORCE_CAPTAIN_DOWN");
    if (force && force[0] == 49 && force[1] == 0) sForceCaptainDown = true;
    bool expectTreasure = false;
    if (const char* expect = std::getenv("P2_NOCARGO_RECONCILIATION_EXPECT_TREASURE"))
    ...
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL NOCARGO_RECONCILIATION requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    if (!pc_window_init("P2 no-cargo room-bolt reconciliation fixture", 960, 540)) return 3;
    pc_window_center();
    {
        ...
        std::printf("P2_NOCARGO_RECONCILIATION_WINDOW size=%dx%d pos=%d,%d display=%dx%d centered=%d\n",
