#include "../pc_port/pc_p2_groink_clock.h"
...
int main()
{
    ...
    P2GroinkSourceClock clock;
    ok &= check(clock.step(0.20, true) == P2GroinkSourceClock::kMaximumTicks,
    ...
    clock.step(1.0 / 60.0, true);
    clock.reset();
    ok &= check(clock.step(1.0 / 60.0, true) == 0, "explicit reset clears stage debt");

    if (ok) std::puts("p2_groink_clock_test PASS");
    return ok ? 0 : 1;
}
