...
def main():
    ...
    record={'status':'failed','fixture':file_record(exe),'provenance':file_record(prov),
            ...
            'limitations':['Injected drop entry; no enemy capture','Actor-local fixture state','P1 Attack receiver, not P2 damage fidelity','No water or scene reload acceptance']}
    ...
    try:
        ...
        markers=['DEMON_DROP_BEGIN generation=1 actual_y=-400.000 target_y=-200.000',
                 'DEMON_DROP_BOUNCE generation=1','DEMON_DROP_DAMAGE generation=1 animation_end=1 accepted=1',
                 'DEMON_DROP_CANCEL generation=2','PASS DEMON_DROP_NATIVE']
