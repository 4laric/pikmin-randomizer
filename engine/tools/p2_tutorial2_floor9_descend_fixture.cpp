// Tutorial-2 floor-9 descend fixture (lane tutorial2-floor9-descend,
// #807; downstream consumer #747 p2-cave-tutorial_2-p1-later-floors).
//
// Adapted from the #757 descend-policy fixture pattern (read-only reuse):
// proves staged tutorial floor 9 boots through the extended engine path.
//
// Proves staged tutorial floor 9 boots through the extended engine path (ENTRY_4 floor 9):
...
//
// Modes: --floor 9 boots floor 9 and passes on READY + live squad;
...
bool sForceCaptainDown = false; // env P2_TUTORIAL2_FLOOR9_FORCE_CAPTAIN_DOWN=1: negative-path test only
int sWantFloor = 0;
...
int guardSelfTest() {
    ...
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        ...
        if (down != rows[i].expectDown) {
            std::printf("FAIL TUTORIAL2_FLOOR9 selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
    ...
    std::printf("P2_TUTORIAL2_FLOOR9_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}

class Tutorial2Floor9App final : public PlugPikiApp {
...
public:
    int idle() override {
        ...
        if (++frames > 30000) {
            std::printf("FAIL TUTORIAL2_FLOOR9 timeout want_floor=%d entry_seen=%d observed=%d\n",
        ...
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_TUTORIAL2_FLOOR9_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
        ...
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_TUTORIAL2_FLOOR9_WAIT frames=%d navi=0 floor=%d\n",
        ...
        if (floor <= 0 && observed % 600 == 0) {
            std::printf("P2_TUTORIAL2_FLOOR9_WAIT observed=%d floor=0 alive=%d\n",
                        observed, alivePikis());
            std::fflush(stdout);
        }
        if (floor == sWantFloor && !entrySeen) {
            entrySeen = true;
            std::printf("P2_TUTORIAL2_FLOOR9_ENTRY_READY floor=%d observed=%d\n", floor, observed);
            std::fflush(stdout);
        }
        if (entrySeen) {
            const int alive = alivePikis();
            if (alive > 0) {
                std::printf("P2_TUTORIAL2_FLOOR9_PASS floor=%d squad_alive=%d observed=%d\n",
                            floor, alive, observed);
                std::puts("PASS TUTORIAL2_FLOOR9");
                std::fflush(stdout);
                std::_Exit(0);
            }
        }
...
int main(int argc, char** argv) {
    ...
    if (sGuardNegativeTest) {
        ...
        std::printf("FAIL TUTORIAL2_FLOOR9 negative test did not trip\n");
        std::fflush(stdout);
        return 1;
    }
    if (sCheckEntry) {
    ...
    if (sWantFloor != 9) {
        std::printf("FAIL TUTORIAL2_FLOOR9 want_floor=%d requires 9\n", sWantFloor);
        std::fflush(stdout);
        return 1;
    }
    const char* force = std::getenv("P2_TUTORIAL2_FLOOR9_FORCE_CAPTAIN_DOWN");
    ...
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL TUTORIAL2_FLOOR9 requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    if (!pc_window_init("P2 Tutorial2 floor-9 descend fixture", 960, 540)) return 3;
    pc_window_center();
    pc_settings_init();
    gsys->Initialise();
    pc_settings_p2d_init();
    nodeMgr = new NodeMgr();
    gsys->run(new Tutorial2Floor9App());
    return 0;
}
