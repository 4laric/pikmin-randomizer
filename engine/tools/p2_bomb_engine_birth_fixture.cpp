// REAL engine Bomb/BombOtakara birth fixture (lane bomb-engine-birth-real-native, #691).
//
// Replacement-main harness proving engine-driven birth through the #616
// provider: on every engine idle tick (guard first), the fixture scans LIVE
// tekiMgr actors and drives pc_p2_bomb_engine_birth_poll(), which constructs
// birth records ONLY for live engine-spawned actors whose generator the
// p2-bomb-mgr-birth.txt sidecar registered. No injected actors, no raw-ID
// birth calls, no simulated records, no allowlist bypass: without a live
// matching actor the run fails closed with explicit markers and no PASS.
//
// Replacement-main convention mirrors tools/p2_kurage_runtime.cpp (scenario
// main instead of pc_main.cpp; 960x540 centred window;
// --experimental-pikmin2-room boot). Promotion to a first-class CMake target
// is a documented shared-owner follow-up; the lane runner links this TU
// against the private pikmin_pc graph without editing shared build files.
#include <SDL2/SDL.h>
#include <GL/gl.h>
#include "App.h"
#include "Node.h"
#include "Graphics.h"
#include "GameCoreSection.h"
#include "Generator.h"
#include "Section.h"
#include "NaviMgr.h"
#include "Navi.h"
#include "Piki.h"
#include "PikiMgr.h"
#include "PikiState.h"
#include "Collision.h"
#include "Creature.h"
#include "MoviePlayer.h"
#include "GameStat.h"
#include "PlayerState.h"
#include "gameflow.h"
#include "system.h"
#include "pc_bbft.h"
#include "pc_gpu_preference.h"
#include "pc_gfx.h"
#include "pc_p2_bomb_mgr_birth.h"
// Unity-included provider implementation (established #616/#677 pattern for
// these provider fixtures): the Section 2/3 host functions live in the .cpp,
// so the fixture carries them directly. The lane links NO separate provider
// object, hence no duplicate symbols. P2_BOMB_MGR_BIRTH_NO_HOST is deliberately
// NOT defined: this is the game-linked build.
#include "pc_p2_bomb_mgr_birth.cpp"
#include "pc_p2_bomb_payload_actor.cpp"
#include "pc_p2_bomb_mgr_birth.h"
#include "pc_window.h"
#include "settings/pc_settings.h"
#include "settings/pc_settings_p2d.h"
#include "teki.h"
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <string>

// Captain-safety guard (#632), vendored verbatim from
// scripts/p2_fixture_captain_guard.h (sha256
// d2f678c9eda75e151eb534077dff9e30ad36ae4796881d971bbd09945f3c3474);
// observation-only, equivalent tested guard. The canonical header is consumed
// read-only; this vendored copy exists because a replacement-main TU cannot
// include a Python-tree script header at native build time.
inline bool p2_fixture_captain_down(bool orimaDead, bool deadState, float hp) {
    return orimaDead || deadState || !std::isfinite(hp) || hp <= 1.0f;
}
inline void p2_fixture_require_captain(bool orimaDead, bool deadState, float hp, int tick) {
    if (!p2_fixture_captain_down(orimaDead, deadState, hp)) return;
    std::printf("P2_FIXTURE_CAPTAIN_DOWN tick=%d hp=%.3f orima_dead=%d dead_state=%d outcome=BLOCKED\n",
                tick, hp, int(orimaDead), int(deadState));
    std::fflush(nullptr);
    std::_Exit(86); // interrupted observation, never a successful fixture exit
}

namespace {
bool sGuardSelfTest = false;
bool sGuardNegativeTest = false;
bool sForceCaptainDown = false; // env P2_BOMB_ENGINE_BIRTH_FORCE_CAPTAIN_DOWN=1: negative-path test only

// Engine-independent self test of the vendored guard truth table. Runs before
// any engine boot so it works without assets or a display.
int guardSelfTest() {
    struct Row { bool orima; bool dead; float hp; bool expectDown; };
    const Row rows[] = {
        {false, false, 100.0f, false},
        {false, false, 1.5f, false},
        {false, false, 1.0f, true},
        {false, false, 0.0f, true},
        {false, true, 100.0f, true},
        {true, false, 100.0f, true},
        {true, true, 0.0f, true},
    };
    for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); ++i) {
        const bool down = p2_fixture_captain_down(rows[i].orima, rows[i].dead, rows[i].hp);
        if (down != rows[i].expectDown) {
            std::printf("FAIL BOMB_ENGINE_BIRTH selftest row=%d orima=%d dead=%d hp=%.3f got=%d want=%d\n",
                        int(i), int(rows[i].orima), int(rows[i].dead), rows[i].hp,
                        int(down), int(rows[i].expectDown));
            std::fflush(stdout);
            return 1;
        }
    }
    std::printf("P2_BOMB_ENGINE_SELFTEST_PASS rows=%d\n", int(sizeof(rows) / sizeof(rows[0])));
    std::fflush(stdout);
    return 0;
}

class BombEngineBirthApp final : public PlugPikiApp {
    int frames = 0, observed = 0, births = 0, census = 0;
    bool setupDone = false, moved = false;
    unsigned birthGen = 0;
    float birthX = 0, birthY = 0, birthZ = 0;
public:
    int idle() override {
        int result = PlugPikiApp::idle();
        if (++frames > 30000) {
            std::printf("FAIL BOMB_ENGINE_BIRTH timeout observed=%d births=%d\n",
                        observed, births);
            std::fflush(stdout);
            std::_Exit(2);
        }
        if (!naviMgr || !tekiMgr || !pikiMgr) {
            if (frames % 600 == 0) {
                std::printf("P2_BOMB_ENGINE_WAIT frames=%d navi_mgr=%d teki_mgr=%d piki_mgr=%d\n",
                            frames, int(naviMgr != nullptr), int(tekiMgr != nullptr),
                            int(pikiMgr != nullptr));
                std::fflush(stdout);
            }
            return result;
        }
        Navi* n = naviMgr->getNavi();
        if (!n) {
            if (frames % 600 == 0) {
                std::printf("P2_BOMB_ENGINE_WAIT frames=%d navi=0\n", frames);
                std::fflush(stdout);
            }
            return result;
        }
        // Guard FIRST: immediately after engine idle, before any observation.
        if (sForceCaptainDown)
            p2_fixture_require_captain(true, true, 0.0f, observed);
        else
            p2_fixture_require_captain(GameStat::orimaDead, !n->isAlive(), n->mHealth, observed);
        if (gameflow.mMoviePlayer && gameflow.mMoviePlayer->mIsActive) {
            gameflow.mMoviePlayer->requestSkip();
            return result;
        }
        if (gameflow.mPauseAll || gameflow.mIsUIOverlayActive) return result;
        ++observed;
        if (!setupDone) {
            setupDone = true;
            pc_p2_bomb_mgr_birth_setup();
            std::printf("P2_BOMB_ENGINE_SETUP ready=%d\n", int(pc_p2_bomb_mgr_birth_ready()));
            std::fflush(stdout);
        }
        if (census < 64) {
            // One bounded diagnostic census of the engine-spawned population.
            // Read-only; proves what the room provides, stages nothing.
            Iterator it(tekiMgr);
            CI_LOOP(it) {
                if (census >= 64) break;
                Teki* actor = static_cast<Teki*>(*it);
                if (!actor) continue;
                const unsigned gen = (actor->mGenerator != nullptr)
                    ? actor->mGenerator->_70 : 0u;
                Vector3f pos = actor->getPosition();
                std::printf("P2_BOMB_ENGINE_CENSUS idx=%d generator=%u type=%d alive=%d "
                            "x=%.3f y=%.3f z=%.3f\n",
                            census, gen, actor->mTekiType, int(actor->isAlive()),
                            pos.x, pos.y, pos.z);
                ++census;
            }
            std::fflush(stdout);
        }
        // Engine-driven birth scan: every live actor is offered to the birth
        // arm, which fires ONLY on live registered carriers. No raw IDs, no
        // injection: unregistered actors simply do not birth.
        Iterator scan(tekiMgr);
        CI_LOOP(scan) {
            Teki* actor = static_cast<Teki*>(*scan);
            if (!actor || !actor->mGenerator) continue;
            if (pc_p2_bomb_engine_birth_poll(actor)) {
                ++births;
                if (birthGen == 0) {
                    birthGen = actor->mGenerator->_70;
                    Vector3f pos = actor->getPosition();
                    birthX = pos.x;
                    birthY = pos.y;
                    birthZ = pos.z;
                }
            }
        }
        if (births > 0 && !moved) {
            // Autonomous-motion proof: the born carrier must move under its
            // own engine AI before any PASS (a static record proves nothing).
            Iterator rescan(tekiMgr);
            CI_LOOP(rescan) {
                Teki* actor = static_cast<Teki*>(*rescan);
                if (!actor || !actor->mGenerator || !actor->isAlive()) continue;
                if (actor->mGenerator->_70 != birthGen) continue;
                Vector3f pos = actor->getPosition();
                const float dx = pos.x - birthX, dy = pos.y - birthY, dz = pos.z - birthZ;
                if (dx * dx + dy * dy + dz * dz > 0.25f) {
                    moved = true;
                    std::printf("P2_BOMB_ENGINE_MOVED generator=%u dx=%.3f dy=%.3f dz=%.3f\n",
                                birthGen, dx, dy, dz);
                    std::fflush(stdout);
                }
            }
            if (moved) {
                std::printf("P2_BOMB_ENGINE_BIRTH_PASS births=%d observed=%d\n",
                            births, observed);
                std::puts("PASS BOMB_ENGINE_BIRTH");
                std::fflush(stdout);
                std::_Exit(0);
            }
        }
        if (observed >= 3600 && births == 0) {
            std::printf("FAIL BOMB_ENGINE_BIRTH no_birth_observed observed=%d\n", observed);
            std::fflush(stdout);
            std::_Exit(1);
        }
        return result;
    }
};
} // namespace

int main(int argc, char** argv) {
    for (int i = 1; i < argc; ++i) {
        if (std::string(argv[i]) == "--guard-self-test") sGuardSelfTest = true;
        if (std::string(argv[i]) == "--guard-negative-test") sGuardNegativeTest = true;
    }
    if (sGuardSelfTest) return guardSelfTest();
    if (sGuardNegativeTest) {
        // Exercise the exact interruption call idle() uses: must print
        // P2_FIXTURE_CAPTAIN_DOWN and exit BLOCKED (86) with no PASS.
        // Engine-independent; the exit code is the assertion.
        p2_fixture_require_captain(true, true, 0.0f, 0);
        std::printf("FAIL BOMB_ENGINE_BIRTH negative test did not trip\n");
        std::fflush(stdout);
        return 1;
    }
    const char* force = std::getenv("P2_BOMB_ENGINE_BIRTH_FORCE_CAPTAIN_DOWN");
    if (force && force[0] == '1' && force[1] == '\0') sForceCaptainDown = true;
    SDL_setenv("SDL_AUDIODRIVER", "dummy", 1);
    SDL_SetMainReady();
    pc_gpu_preference_apply();
    _putenv_s("PIKMIN_RANDOMIZER_TEST_BACKGROUND", "1");
    pc_bbft_init(argc, argv);
    if (!pc_pikipelago_room_preview()) {
        std::printf("FAIL BOMB_ENGINE_BIRTH requires --experimental-pikmin2-room\n");
        std::fflush(stdout);
        return 3;
    }
    if (!pc_window_init("P2 Bomb engine birth fixture", 960, 540)) return 3;
    pc_window_center();
    {
        SDL_Window* window = SDL_GL_GetCurrentWindow();
        int width = 0, height = 0, x = 0, y = 0;
        SDL_GetWindowSize(window, &width, &height);
        SDL_GetWindowPosition(window, &x, &y);
        SDL_Rect bounds{0, 0, 0, 0};
        SDL_GetDisplayBounds(SDL_GetWindowDisplayIndex(window), &bounds);
        const bool centered = std::abs(x - (bounds.x + (bounds.w - width) / 2)) <= 2
            && std::abs(y - (bounds.y + (bounds.h - height) / 2)) <= 2;
        std::printf("P2_BOMB_ENGINE_WINDOW size=%dx%d pos=%d,%d display=%dx%d centered=%d\n",
                    width, height, x, y, bounds.w, bounds.h, int(centered));
        std::fflush(stdout);
    }
    pc_settings_init();
    gsys->Initialise();
    pc_settings_p2d_init();
    nodeMgr = new NodeMgr();
    gsys->run(new BombEngineBirthApp());
    return 0;
}
