...
// surely the tekiMgr is fine...
	...
	f32 minDist = C_PIKI_PARM(mPiki, mBombTargetSearchRange);
	mTarget     = nullptr;
	CI_LOOP(iterTeki)
	{
...
int ActPutBomb::exeSet()
{
	...
	if (dist <= C_PIKI_PARM(mPiki, mBombSetDistance) && absF(angle) < PI / 10.0f) {
		warnPikis();
...
void ActPutBomb::warnPikis()
{
	...
	CI_LOOP(iter)
	{
		...
		if (piki != mPiki) {
			...
			if (dist <= C_PIKI_PARM(mPiki, mBombWarnPikiRange)) {
...
void ActPutBomb::initAim()
{
	...
	mState = STATE_Aim;
	warnPikis();
	mAimTimer   = C_PIKI_PARM(mPiki, mBombAimTime);
	mPlaceTimer = 0.0f;
}
...
int ActPutBomb::exeAim()
{
	...
	if (distanceToTarget > C_PIKI_PARM(mPiki, mBombThrowMinDistance) && distanceToTarget < C_PIKI_PARM(mPiki, mBombThrowMaxDistance)) {
	...
	if (distanceToTarget < C_PIKI_PARM(mPiki, mBombThrowMinDistance)) {
		dirToTarget.multiply(-1.0f);

		mPlaceTimer += gsys->getFrameTime();
		if (mPlaceTimer >= C_PIKI_PARM(mPiki, mBombThrowTimeoutTime)) {
			initSet();
			return ACTOUT_Continue;
		}
	} else {
...
int ActPutBomb::exeThrow()
{
	...
	if (mAnimationFinished) {
		if (mPiki->isHolding()) {
			...
			Vector3f throwVel = getThrowVelocity(catchPos, C_PIKI_PARM(mPiki, mBombThrowSpeed), centre, vel);
			InteractRelease release(mPiki, 1.0f);
			Creature* held = mPiki->getHoldCreature();
			held->stimulate(release);
