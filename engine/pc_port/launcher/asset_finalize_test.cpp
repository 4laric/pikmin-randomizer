...
int main()
{
    ...
    auto denied = std::make_error_code(std::errc::permission_denied);
    ...
    calls = 0; delays.clear();
    ec = retryAssetRename([&] { ++calls; return denied; }, wait);
    check(ec == denied && calls == 5 && delays == std::vector<int>({250, 500, 750, 1000}), "bounded exhaustion");
    calls = 0; delays.clear();
    auto missing = std::make_error_code(std::errc::no_such_file_or_directory);
    ec = retryAssetRename([&] { ++calls; return missing; }, wait);
    check(ec == missing && calls == 1 && delays.empty(), "permanent failure is immediate");

    namespace fs = std::filesystem;
    const auto root = fs::temp_directory_path() / ("nectar finalize test " + std::to_string(steady_clock::now().time_since_epoch().count()));
    ...
    { std::ofstream file(partial / "marker"); file << "preserve me"; }
    fs::create_directory(final);
    check(finalizeAssets(partial, final) == std::errc::file_exists, "existing destination rejected");
    ...
    std::thread release([&] { std::this_thread::sleep_for(milliseconds(300)); CloseHandle(lock); });
    ec = finalizeAssets(partial, final);
    release.join();
#else
    ec = finalizeAssets(partial, final);
#endif
    check(!ec && fs::exists(final / "marker") && !fs::exists(partial), "real directory finalization");
