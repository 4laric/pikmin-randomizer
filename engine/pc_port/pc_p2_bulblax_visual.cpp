...
void pc_p2_bulblax_visual_draw(Graphics& gfx){
 ...
 if(btkReady){const float src=retail.empty()?float(clocks.begin()->second.frame()):retail.begin()->second.player.frame();
  float bs[3],br[3],bt[3];const float bf=btk.duration>0?std::fmod(src,float(btk.duration)):src;
  if(p2btk::sample(btk,btkMatrix,bf,bs,br,bt)){
   // Feed the sampled TEXMTX0 SRT into the engine's marked envmap stage (#239).
   for(int r=0;r<2;++r){const float rr=br[r]*0.0174532925199433f;gfx.mP2EnvSrtOverrideVal[r][0]=bs[r]*std::cos(rr);gfx.mP2EnvSrtOverrideVal[r][1]=-bs[r]*std::sin(rr);gfx.mP2EnvSrtOverrideVal[r][2]=bt[r];}
   gfx.mP2EnvSrtOverride=true;
   if(!btkLogged){btkLogged=true;std::printf("P2_BULBLAX_BTK matrix=%d source_frame=%.3f scale=%.4f,%.4f,%.4f rot=%.4f,%.4f,%.4f trans=%.4f,%.4f,%.4f\n",btkMatrix,bf,bs[0],bs[1],bs[2],br[0],br[1],br[2],bt[0],bt[1],bt[2]);}
  }}
