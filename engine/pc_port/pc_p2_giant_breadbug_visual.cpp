...
Clip clips[2];Shape* nest=nullptr;std::vector<Display> displays;unsigned started=0;bool logged=false;size_t loadedBytes=0;
void fail(){std::fputs("P2_GIANT_BREADBUG_VISUAL invalid profile/model\n",stderr);std::abort();}
Shape* load(const std::string& name){
    ...
    if(size<=0||size>2*1024*1024||loadedBytes+size_t(size)>32*1024*1024)fail();loadedBytes+=size_t(size);in.seekg(0);std::vector<unsigned char> data(size_t(size),0),resources;
...
void pc_p2_giant_breadbug_visual_reset(){for(auto& clip:clips)clip=Clip{};nest=nullptr;displays.clear();started=0;logged=false;loadedBytes=0;}
void pc_p2_giant_breadbug_visual_setup(){
    pc_p2_giant_breadbug_visual_reset();if(!pc_pikipelago_room_preview())return;
    std::ifstream in("p2-giant-breadbug-visual.txt");if(!in)return;std::string word;if(!(in>>word)||word!="P2_GIANT_BREADBUG_VISUAL_1")fail();
    ...
    if(in>>word)fail();
    for(int kind=0;kind<2;++kind)for(size_t i=0;i<clips[kind].frames.size();++i){char name[80];std::snprintf(name,sizeof(name),"lane_ootake_%s1_%03u.mod",kind?"move":"wait",unsigned(clips[kind].frames[i]));clips[kind].shapes.push_back(load(name));}
    nest=load("lane_nest_nest.mod");started=SDL_GetTicks();
    for(const auto& d:displays)std::printf("P2_GIANT_BREADBUG_VISUAL_READY display=%u kind=%d xyz=%.3f,%.3f,%.3f yaw=%.3f behavior=visual_only\n",d.id,d.kind,d.x,d.y,d.z,d.yaw);
}
void pc_p2_giant_breadbug_visual_draw(Graphics& gfx){
    ...
    if(!logged){std::puts("P2_GIANT_BREADBUG_VISUAL_DRAW no_actor_no_collision");logged=true;}
}
