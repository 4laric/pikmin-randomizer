...
APWorld 0.29.0 adds optional bomb_trap_weight 0–10 (default 0), CLI parity and a stable appended item ID for Bomb Ambush. The item replaces filler and uses ItemClassification.trap. Each receipt spawns five BombItems on a radius-65 ring around the captain, starts the native BOMB_Set fuse, and uses normal explosion behavior. All five positions must have dry ground within 25 vertical units; partial allocations are killed without consuming the receipt. The active-gameplay guard excludes menus, cutscenes, pause, day end and dead captains. No Yellow Pikmin gate applies. Cooldown uses active frame time and is max(5 seconds, longest fuse + 2 seconds).

The bomb-ambush-v1 capability uses BENEFITS modes 5–8, appending receipt slot 6 after the reserved bomb delivery slot. Old modes 1–4 are unchanged. PIKMIN_CAMPAIGN_3 stores five consumed counters; older seeds retain their existing checkpoint layout. Duplicate AP receipts do not duplicate effects. Saved consumption survives restart; unsaved effects replay with the unsaved campaign, consistently with other consumables.

Validation: Release game/probe build; 100 Python tests + 8 subtests; compiled trap protocol across all four trap/bomb/captain mode combinations with duplicate receipts, saved consumption, unsaved rollback, reload and retraction rejection; legacy captain and consumable checkpoint regression. Thirty packaged AP fills across all five areas passed, verified trap classification and default-disabled pool. A real-game fixture queued a receipt during pause, observed five lit bombs after unpausing, verified native fuse/explosion states and consumed receipt (observed maximum fuse 2.08 seconds). Player timing/difficulty remains playtest feedback.


## Diegetic Flower Shower (#94)

Flower Shower now creates five native OBJTYPE_Water nectar drops on dry ground in a radius-50 ring around Olimar, with the normal appearance animation. It no longer writes Pikmin flower state. Native collision, drinking animation and growth perform flowering. The reward can spawn with an already-flowered or empty squad, leaving nectar available for later use. Pauses, menus, cutscenes, day end and dead captains retain the shared active-gameplay guard. Inappropriate terrain or partial allocation leaves the receipt queued; queued showers are separated by five active seconds.

Ground nectar avoids the second allocation performed by falling nectar on landing, so all five drops can be allocated before consuming the receipt. Item name/ID, pool, protocol and campaign layout are unchanged; this behavior applies to existing seeds. Saved consumption and unsaved rollback follow the existing consumable rules. The older TEST_HOOKS benefits fixture now checks spawned nectar and unchanged field population instead of expecting instant flowers.

Validation: Release game/probe build passed. The isolated live fixture verified pause deferral, five spawned nectar drops, no instant flowering, and physical nectar collision followed by drinking and growth to Flower. Existing consumable and bomb-trap protocol tests passed (duplicate receipts, checkpoint persistence, rollback and retraction rejection). No Python pool/protocol changes or APWorld version bump were required.


## Smoky Progg Ambush (#95)

Optional progg_trap_weight 0–10 (default 0) replaces filler with a new stable appended Smoky Progg Ambush item, classified as an AP trap. YAML and CLI expose the same weight. The progg-ambush-v1 capability extends BENEFITS to modes 9–16; receipt index 7 and consumed index 5 use PIKMIN_CAMPAIGN_4. Existing modes and checkpoint layouts remain unchanged. Reserved bomb/bomb-trap receipt slots are retained even when those trap types are disabled.

Enabled seeds preload TEKI_Dororo before TekiMgr::startStage on every area. The runtime selects dry ground 250 units from the active captain within 25 vertical units, requires an Onion for native pathfinding, resets the generated Teki personality and starts the standard strategy. It has no generator/check binding. Any living Progg (including a native one) delays another receipt, with an additional 30-second active-play cooldown. The normal appearance, poison, route AI and lifespan remain in control. Runtime actors do not carry into a new day; save consumption and unsaved rollback follow existing consumables.
...
## Launcher window and disc-image extraction (#93)

launcher/gui.py is a tkinter window (Pikmin palette: soil, leaf, the three Onion colors) that Play.cmd now opens through pythonw when available; Play.cmd --console keeps the text launcher. It shows a seed card (slot, mode, start area/color, goal, check count, DeathLink unit, fingerprint), takes either a Pikmin .iso/.gcm or an extracted assets folder, shows AP server/password fields for AP seeds, streams the runner log, and can stop the game. launcher/discimage.py drives the engine's own installer (bin/nectar-launcher.exe, CMake target pikmin_launcher) with --rom/--install-dir/--extract-only, writing to %APPDATA%/PikminRandomizer/game-data/assets and reusing a complete extraction (its .pikmin-assets marker); progress lines feed the window's bar. The console launcher accepts --assets <image> too and now reconfigures stdout with errors=replace because the installer prints non-ASCII file names. scripts/package_release.py packages nectar-launcher.exe when it sits next to nectar.exe and gained --python-install for copying a python.org CPython (with tkinter) as the bundled runtime, self-checked by importing tkinter and websockets with the copied interpreter.

Validation: tests/test_discimage.py (fake installer: progress, reuse, failure, override persistence), tests/test_gui.py (real Tk window: card rendering, AP panel, source status, Play preconditions), 125 tests total; a real extraction of Pikmin USA Rev 1 (638 MB) via nectar-launcher, and the console launcher extracting the same image into a fake APPDATA and reaching PIKMIN_WORLD_RENDERED. --python-install is untested because no python.org install exists on this machine yet.

Bundled runtime validated (#93): package_release.py --python-install with the python.org 3.14.7 install manager layout (Tcl 9 in-DLL, no tcl folder) produces an 18 MiB zip with an isolated ._pth runtime; the packaged launcher started the game and overlay from a folder with spaces with only System32 on PATH, and Play.cmd opened the window through the bundled pythonw. Two defects found only by that isolation: the MinGW GCC runtime DLLs were missing from bin/ (now copied and audited), and Play.cmd broke on an unquoted interpreter path. AP fill tests and the packaged apworld still run under 3.12; the bundled runtime only needs the runner, overlay, tracker and launcher, which import cleanly on 3.14.
