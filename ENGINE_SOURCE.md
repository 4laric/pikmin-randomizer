...
`engine/` is a source-only snapshot of the isolated Open Nectar/BBFT-derived native checkout used for this randomizer, at native commit `165ff3c5`. It includes the inherited native compatibility and adapter code, standalone randomizer integration, health-gauge fix, guarded day-end saves, collection-check hooks, configurable starting Flarlic, and seeded per-color stats, progressive AP stat upgrades stacked on optional wider initial rolls, weighted carrying, scalable check storage and permanent-structure observers. The native source history is retained locally; this directory is a snapshot, not a submodule.
