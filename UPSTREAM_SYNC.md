...
- Current maintained downstream native commit: `568ec58f` (see ENGINE_SOURCE.md); per-color stats and progressive upgrades added after integration in #28/#29.
...
The integration executable is `output/native-upstream-26/build-sync/bin/nectar.exe`, SHA-256 `A6F7D3AE733751F6345A6D984E98D7B55E65C2942DDE639190C20817AFB89754`. The current #29 production executable is `native/build-stats/bin/nectar.exe`, also packaged in `output/turkey-upgrades-01/bin/nectar.exe`, SHA-256 `DCAD0FC2811A139228126C97E77EA43EC9FA5BA848B303F5A519370B50912A61`; it retains this upstream integration and adds progressive per-color AP upgrades. See DEVELOPMENT.md for its validation. Future packages should use the current executable or rebuild current source; older `native/build-randomizer` binaries predate this integration until rebuilt. Local test evidence is under `output/sync26-*` and `output/stats28-*` / `output/stats29-*` and is excluded from GitHub.
