...
Batch #113: [Purple preview](PIKMIN2_PURPLE_PREVIEW.md). Opt-in Purple species metadata, actual source model with sampled idle/walk/attack poses, separate ten-unit carrying strength, source numeric stats and Violet conversion are implemented. The isolated Atlas scenario exercises a 101-strength load. [Landing earthquake and Red dwarf stun](PIKMIN2_PURPLE_IMPACT_GATE_AB.md) are implemented; [direct landing hit progress](PIKMIN2_PURPLE_DIRECT_HIT.md) is recorded separately. [HipDrop flight, source throw/fall poses and native feedback](PIKMIN2_PURPLE_FLIGHT.md) now pass scripted full-throw ground recovery and adult direct-hit checks. Original P2 particle/audio fidelity, full controller acceptance, remaining animation coverage and ship inventory remain open. The [Purple impact specification](PIKMIN2_PURPLE_IMPACT_SPEC.md) tracks implementation gates in #393.

White species work under #131 follows the [White Pikmin specification](PIKMIN2_WHITE_SPEC.md), tracked in #395. The [species/Ivory foundation](PIKMIN2_PURPLE_WHITE_INTEGRATION.md) and [swallowed poison](PIKMIN2_WHITE_POISON.md) have bounded native evidence. Gas, buried work, storage and full campaign persistence remain open.
...
Combined follow-up: [Snow and transition playtest](PIKMIN2_COMBINED_PLAYTEST.md) integrates opt-in source enemy visuals and marker-based descent/exit. [Content manifest](PIKMIN2_CONTENT_MANIFEST.md) prepares all three treasures independently; the roster follow-up below integrates their native receipts.
...
Transition follow-up: [imported hole/geyser models](PIKMIN2_TRANSITION_ASSETS.md)
replace the engineering markers in an opt-in bundle, retaining F6 and tested
checkpoint semantics. Both models were inspected in native renders; two-floor
save/reload tests pass. Geyser effects/animation, physical transition actors and
the surface entry/return pocket remain open. Original Atlas source-position
hauling still stalls uphill (#123); the validated placement is retained.
