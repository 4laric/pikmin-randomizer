...
- Private build: `C:/Users/alari/pikmin-randomizer/output/p2-upstream433-build`, Ninja/MinGW Release with JAudio. The first broad production build passed. The expanded build passed after correcting the Titan save-test link dependencies; the final Crawbster relink is recorded below.
...
Final verification and combined runtime evidence are appended after completion. Builds, logs, fixture data and saves stay private under `output/p2-broad437-*`.
...
4. Fresh in-process gameplay re-entry now has worker acceptance with the scripted-pad harness; acceptance on this full combined pair remains separate. A worker's MapSelect experiment dereferenced a stale object after stage exit; the safe scene-generation signal and scripted-pad harness are now integrated. Process restart and manual manager reset are different gates.
