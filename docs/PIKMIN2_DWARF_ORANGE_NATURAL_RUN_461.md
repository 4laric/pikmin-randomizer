...
## Gate prerequisite remains blocked

Current nativeae00c510 route run `output/qa-dwarf-orange/scene-reentry-08-route-v4/evidence.json` reached the 300-second deadline with waypoint92 closed and 20 Pikmin in BreakwallMode. Earlier attempts placed the captain beside an unrelated live enemy and entered results UI; that staging was corrected to the landing Onion. A subsequent direct-health probe (`scene-reentry-08-route-v5`) observed gate health18/18 unchanged while the captain remained100 and the squad was in wall-work mode. Work-mode entry is therefore not evidence of wall damage. No gate completion or fresh delivery is claimed, and there is no route flag/health override.

Next production/fixture diagnosis belongs to #440: verify actual gate collision contact and attack-animation event delivery before changing damage or route semantics. Live-actor same-process reentry remains PASS on nativeae00c510; its independent historical-checkpoint evidence is unaffected.
