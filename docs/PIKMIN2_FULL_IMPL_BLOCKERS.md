...
|---|---|---|---|
| Ground invertebrates (#165) | Native display; Sokkuri + Armor + ElecBug + TamagoMushi source FSMs done (#407) | Source FSM for Imomushi and Hana (Imomushi plant eat/climb; Hana `ChappyBase` FSM + wake); remaining animation-event execution (damage/drop/flick); ElecBug two-beetle partner link; Imomushi climb/attack; TamagoMushi manager group birth + true Astonish panic; corpse/carry/reward; Piklopedia; cleanup/re-entry; Armor `dmg1` receiver rule and bridge states | #165, #346, #388, #407 |
