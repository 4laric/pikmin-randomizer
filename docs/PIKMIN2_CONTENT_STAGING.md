...
- `install_layout(run, layout, content_root, actor_bindings=None,
  ...
  `validate(source)` pre-flight (which checks the source's declared identity and,
  for Dwarf Orange, the bank↔profile `reference_sha256` binding) and the actor
  binding — before the destination is created, so an unknown identity,
  source-id/enum disagreement, wrong-source hash, missing/wrong source or missing
  actor binding raises `StagingError` and leaves nothing behind. A matching
  on-disk `p2-binding-receipt.json` (no `cache_dir`) or a session-level cache
  marker (with `cache_dir`) is a cached replay that is reused without re-reading
  sources, materializing the staged content into the new run; a conflicting/partial
  tree fails closed, and the cache marker is written last so an interrupted copy
  is never reused as a replay.
