"""#445 Lane 24 Queen-as-Creature slice: bind the Empress Bulblax (30) to a
generated host Teki so ordinary free Pikmin kill it through the engine.

The native sidecar `pc_p2_queen_teki` binds a generated TEKI_Chappy host (the same
carcass path the preview Pod already rebind-scans), rewrites the host health to
the Empress's 5000, draws the Queen model over it, and applies the source
stuck/shake-off thresholds to any Pikmin actually attached in AttackMode. The engine
...
from experimental.pikmin2_lane24_gates import queen_creature_validate
...
public:int idle() override {
 int result=PlugPikiApp::idle();require(++frames<30000||hold,"Queen creature startup timeout");
 ...
 if(ready==1){
  ...
  Iterator it(tekiMgr);CI_LOOP(it){Teki* t=static_cast<Teki*>(*it);if(t&&pc_p2_queen_teki_is_bound(t))host=t;}
  require(host,"Queen host found");
  ...
  SDL_SetWindowTitle(SDL_GL_GetCurrentWindow(),"Empress Bulblax creature host (#445)");
  std::printf("P2_QUEEN_CREATURE_BASELINE red=%d\n",red);
 }
 if(ready==30&&!armed){
  require(cameraMgr&&cameraMgr->mCamera,"Queen camera missing");auto* target=new QueenCameraTarget();target->mSRT.t=Vector3f(host->mSRT.t.x,host->mSRT.t.y+60.f,host->mSRT.t.z);auto* camera=cameraMgr->mCamera;camera->setTarget(target);camera->mControlsEnabled=false;
  ...
  armed=true;std::puts("P2_QUEEN_CREATURE_ARMED no_injection=1 deploy_once=1");
 }
 if(armed&&!finished){
  // Lane-19/21 recipe: ring the live FreeMode reds around the Empress while it
  ...
  if(pc_p2_queen_teki_dead_key_seen()&&!deathSeen){deathSeen=true;std::puts("P2_QUEEN_CREATURE_DEATH_SEEN receiver=engine host_health=0");}
  if(deathSeen&&!corpseSeen){
   Iterator pellets(pelletMgr);CI_LOOP(pellets){Pellet* pl=static_cast<Pellet*>(*pellets);if(pl->isAlive()&&pl->mPelletView==static_cast<PelletView*>(host)){corpseSeen=true;carcass=pl;std::puts("P2_QUEEN_CREATURE_CORPSE_PELLET found=1");break;}}
  }
  if(carcass){
   ...
   if(ready%60==0){
    ...
    std::printf("P2_QUEEN_CREATURE_CARRY carcass_state=%d carry=%d transport=%d nearest=%.1f pokos=%d\n",int(carcass->getState()),int(carcass->mConfig->mCarryMinPikis()),transport,nd,pc_p2_preview_pokos());
    (void)nearest;
   }
  }
  if(deathSeen&&pc_p2_preview_pokos()>0){
   capture("queen-creature-dead.ppm");
   std::printf("P2_QUEEN_CREATURE_RECEIPT pokos=%d corpse_pellet=%d\n",pc_p2_preview_pokos(),corpseSeen?1:0);
   std::puts("PASS P2_QUEEN_CREATURE_RUNTIME");
   std::fflush(nullptr);finished=true;if(!hold)std::_Exit(0);
  } else if(ready>=18000){
   std::printf("FAIL P2_QUEEN_CREATURE_NO_RECEIPT corpse_pellet=%d pokos=%d\n",corpseSeen?1:0,pc_p2_preview_pokos());
...
QUEEN_WANTED = ('wait1', 'dead')


def queen_model_files(bank):
    """Queen walk/dead sampled banks for the sidecar draw. Returns {fname: bytes}."""
    bank = Path(bank)
    meta = json.loads((bank / 'bulblax-bank.json').read_bytes())
    motions = meta['motions']['Queen']
    files = {}
    for name in QUEEN_WANTED:
        motion = motions.get(name)
        if motion is None or motion['poses'] < 1:
            raise ValueError('Bank motion Queen/%s missing' % name)
        for i in range(motion['poses']):
            fname = 'bulblax_Queen_%s_%02d.mod' % (name, i)
            data = (bank / 'Queen' / fname).read_bytes()
            if builder.sha256(bank / 'Queen' / fname) != meta['file_sha256'][fname]:
...
def stage(assets, bank, pod_package, output):
    ...
    # Queen host TEKI (the Empress).
    row = bytearray(enemy)
    struct.pack_into('<I', row, 8, HOST_GENERATOR)
    row[16:48] = b'Queen host Chappy'.ljust(32, b'\0')
    row[80] = HOST_TYPE
    row[81] = 0  # pellet kind none (the borrowed iket personality wants a pellet)
    row[82] = 0  # pellet color
    struct.pack_into('<f', row, 119, 0.0)  # FLT_PelletAppearChance: Empress drops none
    write_position(row, SPAWN)
    entries.append(bytes(row))
    # Starting red squad (64: the Queen's carcass needs 20-30 carriers, plus margin).
    for i in range(SQUAD):
        r = bytearray(piki)
        struct.pack_into('<I', r, 8, 235300 + i)
        r[16:48] = b'Queen creature squad'.ljust(32, b'\0')
    ...
    (run / 'queen-stage.json').write_bytes((json.dumps(
        dict(scene='original P1 Impact Site', interactive=True, host_generator=HOST_GENERATOR,
             ...
             sidecar='p2-queen-teki.txt', pod_package=package['path']), indent=2) + '\n').encode())
    return run
...
def run(assets, bank, pod_package, output, exe):
    ...
    directory = stage(assets, bank, pod_package, output / 'queen')
    ...
    evidence = queen_creature_validate(text, code)
    ...
    print('queen-creature', evidence['passed'], directory, flush=True)
    return evidence
