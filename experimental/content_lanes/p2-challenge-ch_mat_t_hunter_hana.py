"""P0 import-contract adapter for ch_MAT_t_hunter_hana (lane p2-challenge, #547).

Isolated per-source contract: reads the actual flower-garden hunter
caveinfo definition, its referenced unit pool, and its Challenge stage
record from a local disc image; decodes the cave with the EXISTING
shared parsers (experimental.pikmin2_cave_catalog.parse and
experimental.pikmin2_cave.unit_definition, reused and never forked);
parses the user/Matoba/challenge/stages.txt stage block with a small
local reader; and validates everything against the catalogued contract
(expected source hash, 1-floor coverage, stage timers and sprays and
starting roster). Emits a metadata packet and an explicit blocker list.
...
LANE = "p2-challenge-ch_mat_t_hunter_hana"
CAVE_ID = "ch_MAT_t_hunter_hana"
SOURCE = BASE + "/caveinfo/ch_MAT_t_hunter_hana.txt"
EXPECTED_SOURCE_SHA256 = "3bc86de3c581eeccaa8367ba1acbeb4b824c574e7db0ddc3b392f4af6ff8be9a"
...
# Catalogued stage contract (lane entry): starting roster by native
# color and maturity, legacy time, sprays, treasure-count field, UI
# index and per-floor seconds. Asserted verbatim against stages.txt:
# 40 flower blues, 20 flower yellows, 20 flower whites, nothing else.
EXPECTED_STAGE = {
    "cave_file": "ch_MAT_t_hunter_hana.txt",
...
def check_contract(cave, stage):
    ...
    if occupied != want_floors:
        mismatches.append("floor coverage is %s, expected 1" % sorted(occupied))
    if cave["floor_count"] != EXPECTED_FLOOR_COUNT:
        mismatches.append("decoded floor count %d, expected 1" % cave["floor_count"])
    want = EXPECTED_STAGE
...
def build_packet(cave, stage, pools, hashes, mismatches, missing_assets,
    ...
    return {
        ...
        "blockers": [
            ...
            "unadmitted floor roster members (Miulin, Hanachirashi) block "
            "promotion, not this preparatory packet. English stage title "
            "stays unresolved; source ID and UI index are authoritative.",
        ],
        "generated": False,
        "limitations": [
