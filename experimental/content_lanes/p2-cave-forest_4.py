"""forest_4 isolated metadata/import-contract adapter (lane p2-cave-forest_4, #157).
...
source (user/Mukki/mapunits/caveinfo/forest_4.txt, US GPVE01 rev 0) is
...
CAVE_ID = 'forest_4'
SOURCE_PATH = 'user/Mukki/mapunits/caveinfo/forest_4.txt'
ISSUE = 157
LANE = 'p2-cave-forest_4'
...
def validate_manifest(manifest, floors=None, root=None):
    """Check a decoded forest_4 manifest against the canonical baseline.

    The manifest must carry exactly the baseline singleton floors 1..N in
    order, each with the baseline unit pool and the exact baseline token
    sets -- no missing entries, no extras. ``placements``/``actors``/
    ``spawn_layout`` keys are refused outright: weighted definitions must
    never be emitted as fabricated runtime placements. Returns a normalized
    coverage report; raises ContractViolation naming the exact field on any
    deviation.
    """
