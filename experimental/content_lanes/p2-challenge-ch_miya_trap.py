"""P0 import-contract adapter for P2 Challenge 27: ch_MIYA_trap (issue #560).
...
claim. The full content issue #560 stays open for P1/P2.
...
- Loads this lane's entry from docs/PIKMIN_CONTENT_IMPORT_LANES.json and
  the ch_MIYA_trap stage entry from docs/PIKMIN2_CONTENT_INVENTORY.json
  challenge stages (fail closed when either is missing or mismatched).
- Audits the catalogued challenge-stage metadata: exactly 1 floor, one
  positive finite timer (300.0), a 7x3 native color/maturity starting
  population matrix of nonnegative integers (only [3][2] == 25, total
  25), sprays (2/2), treasure field, legacy time, ui_index (26) and table
  order (27). Catalogued values are the baseline; nothing is re-derived.
- Verifies raw source bytes against the pinned sha256 (e5b2a21c...) and
  reports the exact missing prerequisite while the retail definition is
  unstaged (no ISO in this worktree).
- Decodes actual retail definition text with the shared parser
  ...
  then checks decoded 1-floor coverage and summarizes per-floor unit
  pools and token counts. Counts and pools are definition inputs, never
  spawn instances, weights-as-counts or placements.
- Checks resource closure of decoded unit pools against a caller-supplied
  disc file set (units/<pool> plus per-unit arc/texts archives once unit
  tables are decoded), reporting every missing file.
...
LANE = "p2-challenge-ch_miya_trap"
ISSUE = 560
SOURCE_ID = "ch_MIYA_trap"
SOURCE_PATH = "user/Mukki/mapunits/caveinfo/ch_MIYA_trap.txt"
EXPECTED_SHA256 = "e5b2a21c9ea00213996fe7789e24defb106c250c3100a28102c87401efbc2791"
SCHEMA = "p2-challenge-ch_miya_trap-p0/1"
EXPECTED_FLOORS = 1
EXPECTED_FLOOR_SECONDS = [300.0]
EXPECTED_TOTAL_PIKMIN = 25
EXPECTED_NONZERO_CELLS = [(3, 2)]
...
def find_stage_entry(inventory_doc):
    """Return the ch_MIYA_trap entry from a content inventory doc."""
...
def source_prerequisite(source_root=None):
    ...
    The retail definition user/Mukki/mapunits/caveinfo/ch_MIYA_trap.txt is
    not staged in this worktree. When source_root holds it, report its
...
def summarize_decoded(parsed):
    """Check decoded 1-floor coverage; summarize pools and token counts.
