"""P0 source audit and import contract for ch_MAT_crawler (issue #562).

Lane `p2-challenge-ch_mat_crawler`, category `p2-challenge`. A Challenge stage
is TWO live sources: the cave definition
(`user/Mukki/mapunits/caveinfo/ch_MAT_crawler.txt`, same block family as story
caves; here 2 floors, floor 1 carrying an ItemGateMgr gate) and the stage row
in `user/Matoba/challenge/stages.txt` (version, cave reference, 7x3
PikiCounter roster/maturity, legacy time, bitter/spicy dopes, floor count,
otakara count, 2d UI index, per-floor seconds). This module decodes both,
cross-checks them against the lane contract in
`docs/PIKMIN_CONTENT_IMPORT_LANES.json` and an existing catalog baseline,
verifies unit-pool resource closure (with a live from-disc fallback for pools
the story baseline does not cover), and emits an import packet. The cave half
reuses the shared retail parser (`experimental.pikmin2_cave_catalog.parse`);
the stage-block half is a small strict parser owned here (no shared challenge
decoder exists). No placements, spawns, topology, or gameplay claims are
emitted.
"""
import argparse
import hashlib
...
LANE = "p2-challenge-ch_mat_crawler"
SOURCE_ID = "ch_MAT_crawler"
CAVEINFO_PATH = "user/Mukki/mapunits/caveinfo/ch_MAT_crawler.txt"
...
def verify_cave_contract(cave, contract):
    ...
    per-floor unit pools, enemy/item/gate/cap rosters (recorded, not placed)."""
...
def p1_blockers(coverage, closure):
    ...
    gated = [row["floor"] for row in coverage if row["gates"]]
    return [
        ...
        "ItemGateMgr gate decoding (floor %s, life 4000.0) needs the shared gate/electric "
        "manager owner; the packet records the gate roster only." % ",".join(map(str, gated)),
...
def build_packet(iso_path, research_root, lanes_path, catalog, output):
    ...
    packet = dict(
        ...
        pool_hashes=pool_hashes, closure=closure,
    ...
    path = output / "content-ch-mat-crawler-p0.json"
