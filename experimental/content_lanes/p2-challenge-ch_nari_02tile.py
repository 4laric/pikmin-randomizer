"""P0 source-audit adapter for P2 Challenge 05 ch_NARI_02tile (issue #537).
...
BASELINE = {
    ...
    "pikmin_by_native_color_and_maturity": [[0, 0, 50]] + [[0, 0, 0]] * 6,
    "legacy_time": 0.0,
    "bitter_sprays": 0,
    "spicy_sprays": 5,
...
BLOCKERS = [
    ...
    "Challenge content owner (#137): stage completion and timer/roster stage-"
    "table verification remain with the content owner.",
    "Cave generation seams (#129, lanes 34-51): unit-pool instantiation for "
    "2_units_northF_pool_tile.txt and 1_MIYA_bunki_tile.txt is unproven.",
    "Actor/asset owners (#128, #130, #131): UmiMushiBlind/Jigumo/Demon/ "
    "OniKurage/UmiMushi enemy admission plus Egg/WhitePom cap payloads and "
    "the ahiru/toy/diamond treasure pellet closure are unresolved.",
    "Treasure pellet catalog: otakara/item pelletlist resolution for the "
    "eight observed treasure tokens needs the disc pellet table (#137).",
    "Disc asset presence: unit/light/VRBOX asset existence on the retail "
    "disc is unchecked here; supply an explicit disc catalog to "
    "asset_closure() at P1.",
]
...
def asset_closure(scan, catalog):
    ...
    for floor in scan["floors"]:
        refs = {"unit_pool": "user/Mukki/mapunits/units/" + floor["unit_pool"]}
        if floor["light"] != "none":
            refs["light"] = "user/Mukki/mapunits/" + floor["light"]
...
def import_contract(scan, assets):
    ...
    return {
        ...
        "limitations": [
            "Weighted rows are definition inputs, not final spawn instances "
            "or placements; no coordinates are emitted.",
            "Floor timers, starting Pikmin and sprays are inventory baseline "
            "from the lane plan, not values decoded from these bytes.",
            "Enemy/treasure/cap tokens are observed source strings pending "
            "catalog resolution by the owning lanes.",
        ],
        "generated": False,
    }
