"""P0 import-contract adapter for forest_1 (lane shard-caves-forest-forest1-p0, #154).

Isolated per-source contract: reads the actual forest_1 caveinfo
...
and validates the decoded result against this lane catalogued contract
...
LANE = "shard-caves-forest-forest1-p0"
CAVE_ID = "forest_1"
SOURCE = BASE + "/caveinfo/forest_1.txt"
...
# p2-cave-forest_1): per-floor unit pool plus enemy/treasure source tokens.
# "Queen_radar_a" / "Chappy_donutsichigo_s" are cargo-suffixed source
# tokens: base enemy carrying treasure (native first-underscore split),
# while "KareOoinu_s" (tail not a treasure id) must never split.
EXPECTED_FLOORS = (
    {"first": 1, "last": 1, "unit_pool": "1_units_cent3_tsuchi.txt",
     "enemy_ids": ["UjiB", "UjiA", "UjiA", "UjiA", "Clover", "Tukushi",
                   "KareOoinu_s"],
     "treasure_ids": ["juji_key_fc"]},
    {"first": 2, "last": 2, "unit_pool": "1_units_cent2_tsuchi.txt",
     "enemy_ids": ["BlackPom", "HikariKinoko", "KareOoinu_s"],
     "treasure_ids": []},
    {"first": 3, "last": 3, "unit_pool": "2_ABE_norhiba_blkhiba_tsuchi.txt",
     "enemy_ids": ["Hiba", "Hiba", "HikariKinoko"],
     "treasure_ids": ["donutswhite", "dia_c_green"]},
    {"first": 4, "last": 4, "unit_pool": "2_ABE_mid1_nor3_tsuchi.txt",
     "enemy_ids": ["Chappy_donutsichigo_s", "BlackPom", "Hiba",
                   "HikariKinoko"],
     "treasure_ids": ["dia_b_blue"]},
    {"first": 5, "last": 5, "unit_pool": "1_units_boss_tsuchi.txt",
     "enemy_ids": ["Queen_radar_a", "HikariKinoko"],
     "treasure_ids": []},
)
...
def expected_roster(entry, treasure_ids):
    ...
    enemies, carried = [], []
    for token in entry["enemy_ids"]:
        base, cargo = split_cargo_token(token, treasure_ids)
        enemies.append(base)
        if cargo is not None:
            carried.append(cargo)
    return sorted(enemies), sorted(carried), sorted(entry["treasure_ids"])
...
def collect(iso, decomp_root):
    ...
    for name in PELLET_CONFIGS:
        # archive entries are raw bytes at this pin; the pellet catalog
        # parses shift_jis text (same decode as pikmin2_pod.extract).
