"""P0 import-contract adapter for tutorial_1 (lane cave-tutorial1-p0-source-decode, #114).

Isolated per-source contract: reads the actual Emergence Cave caveinfo
definition plus its two referenced unit pools from a local disc image,
...
and validates the decoded result against the catalogued contract (floor
coverage, unit pools, enemy/treasure rosters per floor). Emits a metadata
packet and an explicit blocker list.
...
contract drift is reported as mismatches, never silently corrected.
"""
...
LANE = "cave-tutorial1-p0-source-decode"
CAVE_ID = "tutorial_1"
SOURCE = BASE + "/caveinfo/tutorial_1.txt"
...
# p2-cave-tutorial_1): per-floor unit pool plus enemy/treasure source
# tokens in listed order. Neither floor carries cargo-suffixed tokens.
EXPECTED_FLOORS = (
    {"first": 1, "last": 1, "unit_pool": "1_units_north_tutorial_snow.txt",
     "enemies": ["YellowKochappy"],
     "treasures": ["tape_yellow", "dia_a_red"]},
    {"first": 2, "last": 2, "unit_pool": "1_units_purple_snow.txt",
     "enemies": ["BlackPom", "YellowKochappy", "KareOoinu_s",
                 "KareOoinu_l", "Clover"],
     "treasures": ["map01"]},
)
...
def split_cargo_token(token, treasure_ids):
    ...
    token keeps its full form unless the segment after its FIRST underscore
    is a known treasure. Only the exact `<base>_<treasure>` shape splits.
    """
...
def check_contract(cave, treasure_ids):
    ...
    if occupied != {1, 2}:
        mismatches.append("floor coverage is %s, expected 1-2" % sorted(occupied))
