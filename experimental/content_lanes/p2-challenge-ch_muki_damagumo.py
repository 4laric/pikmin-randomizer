"""P0 source-audit adapter for P2 Challenge 07: ch_MUKI_damagumo (issue #538).
...
- ``BASELINE_RECORD`` is the catalogued stage row from
  docs/PIKMIN2_CONTENT_INVENTORY.json (challenge/stages index 6) and
  docs/PIKMIN_CONTENT_IMPORT_LANES.json. It is the P0 baseline, not a fresh
  decode of the retail caveinfo file.
- The retail file ``user/Mukki/mapunits/caveinfo/ch_MUKI_damagumo.txt`` is
...
SOURCE_ID = "ch_MUKI_damagumo"
SOURCE_PATH = "user/Mukki/mapunits/caveinfo/ch_MUKI_damagumo.txt"
...
LANE = "p2-challenge-ch_muki_damagumo"
ISSUE = 538

EXPECTED_TABLE_ORDER = 9
EXPECTED_UI_INDEX = 6
EXPECTED_FLOORS = 1
...
class StageRecordError(ValueError):
    """A catalogued stage row violates the ch_MUKI_damagumo import contract."""


# Exact catalogued baseline (inventory challenge/stages index 6). Baseline,
# not a fresh source decode; see module docstring.
BASELINE_RECORD = {
    "table_order": 9,
    ...
    return [
        ...
        {"id": "#130/#131", "owner": "Actor/asset/species owners",
         "why": "Full resource closure (caps/helpers/held items) and any "
                "unadmitted damagumo-encounter species block promotion, not P0."},
        ...
        {"id": "display-name", "owner": "Localization audit",
         "why": "English title unresolved; source ID ch_MUKI_damagumo and UI "
                "index 6 stay authoritative, never guessed."},
    ]
...
def audit_packet(record=None, search_roots=()):
    ...
    return {
        ...
        "baseline": "catalogued inventory row (challenge/stages index 6); "
