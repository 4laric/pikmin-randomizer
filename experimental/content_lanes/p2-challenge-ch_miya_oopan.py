"""P0 source-audit / import-contract adapter for P2 Challenge 22: ch_MIYA_oopan.

Lane p2-challenge-ch_miya_oopan, issue #554. Implementation owner: Codex
through shared GitHub account 4laric; contributor Muse Spark 1.3 via OpenCode.

This module owns ONLY this source entry: ``ch_MIYA_oopan`` (p2-challenge),
source ``user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt``. It reuses the
...
``p2-challenge-ch_miya_oopan``; English title unresolved, source ID and UI
index authoritative):

- floors: 1; floor timer: [150.0] seconds
- starting Pikmin by native color/maturity (7x3 matrix): native color rows
  0..4 each ``[5, 0, 0]`` (five young of each of the first five native
  colors), rows 5 and 6 empty. This is a fresh staged starting roster, not a
  runtime observation; the maturity/color mapping is preserved verbatim.
- bitter sprays: 0; spicy sprays: 0; legacy_time: 0.0
- treasure_count_field: 0; ui_index: 21; table_order: 15
...
SOURCE_ID = "ch_MIYA_oopan"
SOURCE_PATH = "user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt"
EXPECTED_SHA256 = "76fad15ebc041af12f05e887ad64c324354d49fc89acad57d043d9881951b4b9"
...
SEARCH_CANDIDATES = (
    "output/pikmin2-runtime/pikmin2-source-test.iso!/user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt",
    "C:/Users/alari/bbft/dist/cohesion/pikmin/assets/dataDir/user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt",
    "native/pikmin2-research/user/Mukki/mapunits/caveinfo/ch_MIYA_oopan.txt",
)
...
def starting_roster(details=None):
    """Summarize the staged starting roster from the 7x3 matrix.

    Returns a list of ``{native_color, maturity, count}`` for non-zero cells
    only. This is metadata staging, not runtime evidence.
    """
    record = validate_details(details)
    roster = []
    for color, row in enumerate(record["pikmin_by_native_color_and_maturity"]):
        for maturity, count in enumerate(row):
            if count:
                roster.append({"native_color": color, "maturity": maturity,
                               "count": count})
    return roster


def floor_coverage(floors, expected=1):
...
def audit(source_bytes=None, details=None, exists=None):
    ...
    report = {
        ...
        "expected_sha256": EXPECTED_SHA256,
        "details": record,
        "starting_roster": starting_roster(record),
        "source": None,
        "definitions": None,
