"""P0 import-contract adapter for P2 Challenge 18: ch_ABEM_LeafChappy (#550).

Isolated metadata/decode boundary for the two-floor timed challenge
stage (user/Mukki/mapunits/caveinfo/ch_ABEM_LeafChappy.txt). It reuses
...
p2-challenge-ch_abem_leafchappy, details) and the inventory
(docs/PIKMIN2_CONTENT_INVENTORY.json challenge.stages); both pin
source_sha256 49cc9076... for the definition file. hash_pin_consistency
...
SOURCE_PATH = 'user/Mukki/mapunits/caveinfo/ch_ABEM_LeafChappy.txt'
...
def hash_pin_consistency(lanes_path, inventory_path):
    ...
    entries = [lane for lane in lanes.get('lanes', [])
               if lane.get('lane') == 'p2-challenge-ch_abem_leafchappy']
    if len(entries) != 1:
...
def validate_stage_metadata(details):
    ...
    if floors != EXPECTED_FLOORS or not isinstance(floors, int):
        raise ValueError('Challenge stage must declare exactly 2 floors')
...
def summarize(metadata, source_sha256):
    ...
    return dict(
        ...
        limitations=['Stage timing/roster/spray metadata is catalogued, not observed gameplay.',
                     ...
                     'P0 packet is metadata for integrator review; full content issue #550 stays OPEN.'])
