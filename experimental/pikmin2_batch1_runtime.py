"""Batch-1 native-display runtime probe: bind, exact birth XYZ and framed draw.
...
``P2_BATCH3_DRAW`` path is otherwise never reached) and then exits.

Visual-only: no source FSM, combat, receivers or lifecycle is claimed.
"""
import argparse
import json
import os
...
public:int idle() override {
 ...
 if(ready==1){
  std::ifstream input("batch1-positions.txt");unsigned id;std::string species;float x,y,z;int count=0;float cx=0,cy=0,cz=0;
  while(input>>id>>species>>x>>y>>z){
   ...
   require(matches==1,"batch1 roster identity");
  ...
  require(count>0,"batch1 roster missing");cx/=count;cy/=count;cz/=count;
 ...
 if(ready>=180){std::puts("PASS P2_BATCH1_RUNTIME birth_bind_camera_framed");std::fflush(stdout);std::_Exit(0);}
...
def readings(text):
    ...
    return births, draws, binds


def run(stage, exe, output=None):
    ...
    evidence = dict(passed=all(checks.values()), checks=checks, exit_code=code,
                    births=births, binds=binds, draws=draws,
                    scene=manifest.get('scene'), total_actors=len(expected),
                    scope='Visual display only: bind, birth XYZ and framed draw. '
                          'No source FSM/combat/lifecycle claim.')
