"""Hash-bound Dwarf Bulbear (KumaKochappy) installation into a private run.
...
visuals are preserved untouched. Native registration (including the
ChappyRelation parent link to KumaChappy, EnemyID 35) belongs to the
...
SOURCE_ID = 76
HEALTH = 500
ACTORS_TXT = 'p2-dwarf-bear-actors.txt'
ACTORS_HEADER = 'P2_DWARF_BEAR_ACTORS_1'
INSTALL_JSON = 'dwarf-bear-install.json'
...
def plan(bank, profile_dir, generator_ids):
    ...
    if (metadata.get('schema'), metadata.get('species'),
        ...
        raise ValueError('Wrong Dwarf Bulbear bank identity')
    ...
    if found or stray:
        if sorted(found) != sorted(expected_names) or stray:
            raise ValueError('Incomplete/unexpected Dwarf Bulbear visual bank')
        ...
        if set(metadata.get('file_sha256', {})) != set(expected_names) or any(
            ...
            raise ValueError('Dwarf Bulbear visual bank hash mismatch')
...
def install(bank, profile_dir, run, generator_ids):
    ...
    if any(t.exists() for t in targets):
        raise ValueError('Refusing existing/conflicting Dwarf Bulbear installation')
    ...
    receipt = dict(schema=1, species=SPECIES, source_id=SOURCE_ID, health=HEALTH,
                   ...
                   parent_relation='ChappyRelation to KumaChappy (EnemyID 35) not installed; parent not imported',
