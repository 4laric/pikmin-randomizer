...
# Slice 2: the buds now bind the batch-2 `flora` family Chappy placement
# vehicles generated by the flora arena. Generators match FAMILIES['flora']
# arena_ids: 353002 BluePom .. 353007 RandPom (353001 is Pelplant, 353008 the
# P1 Chappy control). The base Pom (82) is not in the arena and is a rejection
# probe only.
REDPOM = 353003
RANDPOM = 353007
BASEPOM = 353099
REDPOM_POS = (-120.0, 30.0, 1850.0)
RANDPOM_POS = (360.0, 30.0, 1850.0)
...
public:
	int idle() override {
		...
		if (phase == 0) {
			...
			plan.push_back({alive[0], -120.0f, 30.0f, 1850.0f});   // RedPom, own colour -> refund
			plan.push_back({alive[1], -120.0f, 30.0f, 1850.0f});   // RedPom, other colour -> accept
			plan.push_back({alive[2], 360.0f, 30.0f, 1850.0f});    // RandPom Queen -> accept
...
def stage(assets, imported, output):
    ...
    rejection probe. The buds now produce P2_POM_BIND on their live Chappy host
    and P2_POM_DRAW through the batch-2 draw chain."""
...
def validate(text, code):
    ...
    return dict(passed=not failed, failed=failed, checks=required, blocked=blocked, exit_code=code,
                scope='batch-2 Chappy-vehicle Candypop buds: bind/draw/FSM/death/conservation; base-Pom rejection and capacity-pressure inject are labeled probes')
