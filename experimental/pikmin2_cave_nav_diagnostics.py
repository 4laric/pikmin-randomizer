...
def prepare(source,output):
    ...
    (output/'pc_p2_cave.cpp').write_bytes(result.encode('utf-8'));(output/'pc_p2_cave_nav_diagnostics.h').write_bytes(HEADER.encode('utf-8'))
    ...
    (output/'navigation.patch').write_bytes(diff.encode('utf-8'));(output/'provenance.json').write_text(json.dumps(dict(source=str(source.resolve()),sha256=hashlib.sha256(raw).hexdigest(),scope='isolated patch only',enabled_by='PIKMIN_CAVE_NAV_DIAGNOSTICS=1',limit='at most120rows per stage, minimum2seconds interval'),indent=2))
