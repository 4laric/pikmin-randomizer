...
# Staged markers must self-label staged=1 (sidecar honesty check).
STAGED_MARKER_RE = re.compile(r"P2_JIGUMO573_(?:STAGE|CARRIER|RELATCH|REINFORCE) ")
    ...
    for line in lines:
        ...
        if ("inject" in low or "p2-bombotakara-native" in low or "p2-jigumo-inject" in low):
            if "staged=1" not in line:
                ...
                break
    for line in lines:
        if STAGED_MARKER_RE.search(line) and "staged=1" not in line:
            verdict["failures"].append("unstaged-staged-marker")
            return verdict
...
def main(argv: list) -> int:
    ...
    if args.log is None and (args.assets is None or args.output is None or args.exe is None):
        parser.error("--assets/--output/--exe are required without --log")
