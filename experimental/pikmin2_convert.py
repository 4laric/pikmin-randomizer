...
def decode(data, approximate_materials=False, bake_rigid=False, pose=None, draw_matrices=None, missing_normals="error", singular_normal="error", billboard="error", bindings=None):
    ...
    if billboard not in BILLBOARD_MODES:raise ValueError(f"Unsupported billboard mode {billboard!r}")
...
def convert(source, output, approximate_materials=False, y_offset=0.0, bake_rigid=False, pose=None, material_colors=None, missing_normals="error", singular_normal="error", billboard="error"):
    if pose is not None and not bake_rigid: raise ValueError('Animation pose requires rigid baking')
    report=write_model(decode(Path(source).read_bytes(), approximate_materials,bake_rigid,pose,missing_normals=missing_normals,singular_normal=singular_normal,billboard=billboard),output,str(source),y_offset,material_colors)
