"""Waterwraith / Tyre batch-2 installation into a private run (#352, parent #175).

Thin binding of :mod:`experimental.pikmin2_batch2_core` to the Waterwraith
family config. Consumes the batch-1 ``waterwraith.json`` manifest (policy
``P2_WATERWRAITH_1``). BlackMan (boss) and Tyre (dependent roller child) are
staged as explicit actors; boss phase/reward behavior remains native work
(#175/#186).
"""
import argparse
import json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--actor', action='append', required=True,
                        help='generator:species, e.g. 352001:BlackMan')
