"""ElecBug28 exactly-once ordinary Onion receipt (#585, parent #569).

Bounded follow-on of #495 (ElecBug natural death requested next after the
Sokkuri slice): the preview room runs without a randomizer session by
design, so no receipt could exist yet. This module stages an ElecBug pair
arena, boots a private replacement-main fixture WITH a real session
bootstrap, and validates that a naturally hauled corpse drives the real
ordinary endpoint (GoalItem::suckMe -> pc_randomizer_p2_corpse_delivered)
to ``onion:p2:28`` new=1, then new=0 for the duplicate across a process
restart sharing the session ledger.

Transport PASS needs BOTH the receipt line AND natural-carry markers; a
receipt alone is interface-only (same honesty rule as #495/#578). No enemy
health/state writes, no Transport assignment, no kill and no credit is ever
injected. The single p1-derived staged Purple landing that flips the beetle
into Reverse is labelled ``flip=staged-press`` (the host has no Purple
hipdrop state); death/corpse/haul/receipt remain natural. Re-bind is not
scene re-entry: gate 6 stays UNTESTED.
"""
...
RECEIPT_RE = re.compile(
    r"P2_ORDINARY_P2_RECEIPT seed=(\S+) id=onion:p2:28:(\d+) generator=(\d+) new=([01])")
CARRY_RE = re.compile(
    r"P2_ELECBUG28_CARRY tick=(\d+) moved=([\d.]+) goal_dist=([\d.\-]+) state=(\d+)")
DELIVERED_RE = re.compile(
    r"P2_ELECBUG28_DELIVERED_TO_GOAL tick=(\d+) moved=([\d.]+)")
READY_RE = re.compile(
    r"P2_ELECBUG28_READY squad=(\d+) elecbug_gen=346002 pair_gen=346010 reg=2 session=1")
INJECTED_RES = (
...
def parse_receipts(text):
    """Return every onion:p2:28 receipt marker as a dict."""
...
def check_receipt_once(text, expect_new):
    """Exactly one onion:p2:28 line with the expected new= flag."""
    ...
    if receipts[0]["new"] != expect_new:
        return dict(passed=False,
                    ...
                    receipts=receipts)
    if receipts[0]["generator"] not in (ELECBUG_GEN, ELECBUG_SECOND_GEN):
        return dict(passed=False, reason="receipt generator is not an ElecBug generator",
                    receipts=receipts)
    return dict(passed=True, reason="onion:p2:28 stage=%d new=%d" % (
...
def check_natural_carry(text, min_moved=100.0):
    ...
    if injected:
        return dict(passed=False, reason="injection markers present", best=best)
    if not carries or best < min_moved:
        return dict(passed=False,
                    reason="no natural haul movement (best %.1f)" % best, best=best)
    if not delivered:
        return dict(passed=False, reason="no goal-delivery marker", best=best)
    return dict(passed=True, reason="hauled %.1f to container goal" % best, best=best)


def check_ordinary_corpse_source(retail_root=None):
    # ElecBug keeps the ordinary carcass path (source ElecBug has a normal
    # dead/carcass lifecycle), so a hauled corpse reaching an Onion is
    # source-backed behavior, not a proxy credit.
    root = Path(retail_root) if retail_root else Path(os.environ.get(
        "PIKMIN_P2_RETAIL_ROOT",
        r"C:/Users/alari/pikmin-randomizer/native/pikmin2-research"))
    path = root / "src/plugProjectNishimuraU/ElecBug.cpp"
    try:
    ...
    return dict(passed=passed,
                detail="startCarcassMotion present" if passed else "MARKER ABSENT")
...
def validate(text, expect_new, retail_root=None, code=0):
    ...
    binds = bool(re.search(
        r"P2_ELECBUG_DELIVERY_BIND generator=%d source_id=28" % ELECBUG_GEN, text))
    ...
    flipped = "P2_ELECBUG28_FLIPPED" in text
    dead = bool(re.search(r"P2_ELECBUG28_DIED tick=\d+ health=[\d.]+", text))
    corpse = bool(re.search(r"P2_ELECBUG28_CORPSE pellet=1", text))
    injected = [token for token in INJECTED_RES if token in text]
    no_inject = not injected
    no_extinction = not re.search(r"Extinction", text, re.IGNORECASE)
    completion = "PASS P2_ELECBUG28_RECEIPT_RUN" in text
    checks = dict(
    ...
    return dict(passed=passed, checks=checks, receipt=receipt, carry=carry,
                ...
                natural_vs_injected=dict(
                    ...
                    flip="staged-press", inject_present=not no_inject))


def prepare(assets, imported, output):
    """Stage an ElecBug-pair arena (cargo-free marker keeps the receipt path).

    Same ElecBug pair arena as the #165 natural runtime, with the maintained
    batch2 core. Carriers still have the room's Red container as their goal;
    the randomizer ordinary endpoint (not pc_p2_preview_deliver) grants the
    receipt when the corpse is absorbed.
    """
    from experimental.pikmin2_elecbug_natural_runtime import elecbug_pair_cfg
...
def instrument(source, app=None):
    ...
    if "P2_ELECBUG28_READY" in source:
    ...
    # so pc_randomizer_init is never reached and pc_randomizer_p2_corpse_delivered
    # can never grant. This replacement-main fixture enables the full session
    # itself from the SAME argv before gsys->run().
...
def run_once(session, assets, imported, output, exe, seconds, label):
    ...
    (arena / "bootstrap.txt").write_text((native_run.directory / "bootstrap.txt").read_text())
    atomic_write(arena / "state.txt", session.native_state(native_run.token, True))
...
def run_session(session_dir, seed_name, assets, imported, output, exe, seconds=600):
    ...
    for index, expect in ((1, 1), (2, 0)):
        arena, meta, text = run_once(
            session, assets, imported, output / ("run%d" % index), exe, seconds, "run%d" % index)
        ...
        (arena / "elecbug28-receipt-validation.json").write_text(json.dumps(result, indent=2) + "\n")
