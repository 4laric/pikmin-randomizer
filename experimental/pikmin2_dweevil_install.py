"""Dweevil family batch-2 installation into a private run (#349, parent #170).

Thin binding of :mod:`experimental.pikmin2_batch2_core` to the dweevil family
config. Consumes the batch-1 ``dweevils.json`` manifest (policy
``P2_OTA_DWEEVIL_1``), binds every installed pose by SHA-256 and refuses
conflicting or changed sources before any mutation. Native registration stays
with the integration lead (#186).
"""
import argparse
import json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--actor', action='append', required=True,
                        help='generator:species, e.g. 349001:FireOtakara')
