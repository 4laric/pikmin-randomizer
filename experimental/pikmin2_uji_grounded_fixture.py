...
static bool ujiGroundedFixture(Navi* n){
 ...
 if(step==0){
  ...
  for(int i=0;i<10;++i){int id;float x,y,z;require(bool(poses>>id>>x>>y>>z)&&id==61000+i,"pose identity");auto* t=bugs[i];require(std::fabs(t->mSRT.t.x-x)<.1&&std::fabs(t->mSRT.t.y-y)<.1&&std::fabs(t->mSRT.t.z-z)<.1,"native effective XYZ mismatch");
   std::printf("P2_UJI_GROUNDED id=%d xyz=%.2f,%.2f,%.2f\n",id,t->mSRT.t.x,t->mSRT.t.y,t->mSRT.t.z);
   float health=t->mHealth;require(t->getTekiOption(BTeki::TEKI_OPTION_INVINCIBLE),"initial actor not buried");require(!t->stimulate(InteractAttack(n,nullptr,10000,false))&&t->mHealth==health&&t->mStoredDamage==0,"buried attack accepted");
   walkGoals.push_back(Vector3f(1020,0,0));walkGoals.push_back(Vector3f(x,0,z));
  }
  require(!(poses>>header),"trailing poses");walkGoals.push_back(Vector3f(1020,0,0));secondFloor=true;walkPoint=0;phase=1;n->mKontroller=new FixtureController();capture("uji-grounded-start.ppm");step=1;ticks=0;
 }else if(step==1){
