"""Private original Impact Site arena staging: one Dwarf Bulbear plus P1 control.
...
Dwarf Bulbear specific: the source parent-following behavior (ChappyRelation
to KumaChappy, EnemyID 35, WalkPath state) cannot be staged because Spotty
Bulbear is not yet imported; the arena roster therefore stages the family
actor in standalone visual placement only and defers the relation to the
integration hook request on #186.
"""
import argparse
import hashlib
import json
...
from experimental.pikmin2_dwarf_bear_install import install

IDS = (211101, 211102)
POSITIONS = ((-150., 30., 1850.), (150., 30., 1550.))
SPECIES = ('KumaKochappy', 'P1 Chappy')
# Source yaw: no authored above-ground source placement yaw was audited for
# KumaKochappy (P2 cave spawn); kept as unapplied metadata, never in offsets.
SOURCE_YAW = None
...
def roster(assets):
    ...
    for identity, kind, xyz in zip(IDS, SPECIES, POSITIONS):
        ...
        placements.append(dict(generator=identity, species=kind, native_family='Chappy',
                               ...
                               parent_following='deferred: KumaChappy (EnemyID 35) not imported; '
                                               'WalkPath/ChappyRelation unavailable'))
...
def prepare(assets, bank, profile, output):
    ...
    result = dict(schema=1, scene='P1 Impact Site', stage_slot='chal0', actors=actors,
                  ...
                  limitations=['Source KumaKochappy visuals and health only; proxy P1 AI, not P2 FSM',
                               'No source yaw applied', 'Native registration pending integration lead (#186)',
                               'Parent-following (ChappyRelation -> KumaChappy 35, WalkPath) not staged: '
                               'Spotty Bulbear not imported; standalone placement only',
