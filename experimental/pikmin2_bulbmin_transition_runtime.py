"""Lane 11 Bulbmin leader-dependency cave-transition gate.

Process 1 (phase ``write``) boots floor 1 with a schema-3 entry squad of 18 reds,
converts two of them through the Bulbmin bridge's public entrypoints
(``pc_p2_bulbmin_birth``/``pc_p2_bulbmin_whistle``), leaving one wild (unwhistled)
dependent and one whistled (recruited) dependent, then drives the real
``pc_p2_cave_checkpoint(false)`` at the Research Pod. The engine must emit
``P2_CAVE_BULBMIN_TRANSITION move=descend removed=1`` (the wild dependent is
dropped) and write ``p2-cave-transfer.txt``, then exit 42. The campaign
supervisor parser (:func:`experimental.pikmin2_campaign.transition`) validates
that transfer and re-serializes a schema-3 entry for process 2. Process 2 (phase
``read``) boots floor 2 from that entry and the engine restores the squad; the
fixture requires exactly one live Bulbmin (``P2_CAVE_RESTORE species=5``) and
that it survives the schema-3 restart.

The fixture only schedules the bridge calls and the checkpoint handoff; every
wire byte and restore is produced by the engine. The F6 confirmation dialog is
skipped by passing ``confirm=false``.
"""
import argparse
import json
import os
...
APP = r'''class RoomApp : public PlugPikiApp {
 int frames=0,observed=0;bool converted=false,checked=false;
 static bool is(const char* s,const char* w){return s&&std::strcmp(s,w)==0;}
public:int idle() override {
 int result=PlugPikiApp::idle();require(++frames<120000,"lane11 bulbmin transition startup timeout");
 ...
 if(phase&&is(phase,"read")){
  if(observed>=60){
   ...
   if(bulb>0){std::puts("PASS P2_BULBMIN_TRANSITION_RESTORE");std::fflush(stdout);std::_Exit(0);}
   if(observed>6000){std::puts("FAIL P2_BULBMIN_TX_READ timeout");std::fflush(stdout);std::_Exit(1);}
  }
  return result;
 }
 if(phase&&is(phase,"write")){
  if(!converted){
   Piki* wild=nullptr;Piki* rec=nullptr;
   Iterator p(pikiMgr);CI_LOOP(p){Piki* v=static_cast<Piki*>(*p);if(!v->isAlive()||pc_p2_species(v)!=1)continue;
    if(!wild){wild=v;}else if(!rec){rec=v;break;}}
   if(wild&&rec){
    int madeWild=pc_p2_bulbmin_birth(wild)?1:0;
    int phaseWild=pc_p2_bulbmin_phase(wild);
    std::printf("P2_BULBMIN_TX_WILD made=%d phase=%d\n",madeWild,phaseWild);
    int madeRec=0,phaseRec=-1;
    if(pc_p2_bulbmin_birth(rec)){madeRec=1;if(pc_p2_bulbmin_whistle(rec))phaseRec=pc_p2_bulbmin_phase(rec);}
    std::printf("P2_BULBMIN_TX_RECRUIT made=%d phase=%d\n",madeRec,phaseRec);
    std::fflush(stdout);
    converted=true;
   }
  }
  if(converted&&!checked&&observed>=30&&n->getCurrState()
        &&n->getCurrState()->getID()==NAVISTATE_Walk){
   checked=true;const Vector3f t=pod->mSRT.t;n->mSRT.t=Vector3f(t.x,30.0f,t.z);
   ...
   std::puts("FAIL P2_BULBMIN_TX_CHECKPOINT rejected");std::fflush(stdout);std::_Exit(1);
  }
  if(observed>8000){std::puts("FAIL P2_BULBMIN_TX_WRITE timeout");std::fflush(stdout);std::_Exit(1);}
...
def run(assets, imported, treasure, pod, purple, exe, output, seconds=75):
    ...
    # Phase 1: convert two reds via the bridge (one wild, one whistled) and descend.
    ...
    # Supervisor validation + re-serialize the entry for process 2 (schema 3 now carries a bulbmin).
    ...
    result['read'] = {'run': str(run2), 'capture': {k: meta2[k] for k in
                      ('executable_sha256', 'elapsed_seconds', 'exit_code', 'timed_out')}}
    (output / 'bulbmin-transition-validation.json').write_text(json.dumps(result, indent=2) + '\n')
...
def validate(text1, text2, transfer_text):
    ...
    checks = dict(
        ...
        wild_made=bool(re.search(r'P2_BULBMIN_TX_WILD made=1 phase=0', text1)),
        recruit_made=bool(re.search(r'P2_BULBMIN_TX_RECRUIT made=1 phase=1', text1)),
        transition_dropped_wild=bool(re.search(r'P2_CAVE_BULBMIN_TRANSITION move=descend removed=1', text1)),
        checkpoint_ok=bool(re.search(r'P2_BULBMIN_TX_CHECKPOINT ok=1', text1)),
        transfer_header=transfer_text.startswith('P2_CAVE_TRANSFER_3\n'),
        transfer_floor=bool(re.search(r'\n1 0.625 17\n', transfer_text)),
        transfer_one_bulbmin=(len([l for l in transfer_text.splitlines()[3:] if l.split()[0] == '5']) == 1),
        read_restore=bool(re.search(r'P2_CAVE_RESTORE species=5 maturity=0', text2)),
        read_bulbmin=bool(re.search(r'P2_BULBMIN_TX_READ bulbmin=1', text2)),
        read_pass='PASS P2_BULBMIN_TRANSITION_RESTORE' in text2,
    ...
    return dict(passed=all(checks.values()), checks=checks,
                ...
                limitations=['Engineered arena floor 1 descend; not production cave placement.',
                             'Leader dependency exercised through the Bulbmin bridge public birth/whistle '
                             'entrypoints only; no Mother Bulbmin actor is exercised (its birth is out of scope).'])
