#!/usr/bin/env python3
"""Forest P1 native runtime runner (lane p2-overworld-forest-p1-native-runtime).

Stages a private run layout from the P1 forest manifest + session seed,
...
SCHEMA = "p2-forest-p1-run-1"

REQUIRED_ENGINE_MARKERS = [
    "P2_FOREST_P1_WINDOW",
    "P2_FOREST_P1_ENGINE_FACT",
]
...
BOUNDARY_PASS_MARKERS = {
    "boot_forest_surface": "P2_FOREST_P1_BOOT_PASS",
    "day_transition": "P2_FOREST_P1_DAY_PASS",
    "save_reload": "P2_FOREST_P1_SAVE_PASS",
    "receipt_replay": "P2_FOREST_P1_RECEIPT_PASS",
    "exit_reentry": "P2_FOREST_P1_REENTRY_PASS",
}

FAILURE_MARKERS = (
    "FAIL FOREST_P1_RUNTIME",
    "P2_FIXTURE_CAPTAIN_DOWN",
    "P2_FOREST_P1_UNSUPPORTED",
)
...
def stage_run_layout(manifest_path, seed_path, run_dir, pins):
    ...
    if seed.get("course") != "forest":
        raise ValueError("session seed course is not forest")
    layout = os.path.join(run_dir, "forest-p1")
    ...
    record = {
        ...
        "source_pins": pins,
        "staged_files": ["forest-p1/manifest.json", "forest-p1/session-seed.json"],
    }
...
def validate_run_layout(run_dir):
    ...
    try:
        ...
        if seed.get("schema") != "p2-surface-session-1" or seed.get("course") != "forest":
...
def verify_run_log(log_text):
    ...
    for boundary, marker in BOUNDARY_PASS_MARKERS.items():
        ...
        elif "P2_FOREST_P1_UNSUPPORTED boundary=%s" % boundary in log_text:
    ...
    overall_pass = (
        ...
        and not failures
        and "PASS FOREST_P1_RUNTIME" in log_text
    )
    return {
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Forest P1 native runtime runner")
