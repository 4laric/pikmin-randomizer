"""Dwarf Orange Bulborb (BlueKochappy) sampled pose bank; placement is supplied by the caller.

Builds a deterministic rigid pose bank from an audited
pikmin2_dwarf_orange_profile extraction. Sampled frames always include the
...
HEADER = 'P2_DWARF_ORANGE_BANK_1'
PROFILE = 'P2_DWARF_ORANGE_PROFILE_1\nspecies BlueKochappy\nhealth 250\n'
MODEL = 'dwarf-orange.bmd'
PROFILE_JSON = 'dwarf-orange-profile.json'
BANK_JSON = 'dwarf-orange-bank.json'
BANK_TXT = 'p2-dwarf-orange-bank.txt'
PROFILE_TXT = 'p2-dwarf-orange-profile.txt'
POSE_PREFIX = 'dwarf_orange'
...
def parse_bank(text):
    ...
    if not tokens or tokens[0] != HEADER:
        raise ValueError('Expected Dwarf Orange Bulborb bank')
...
def build(imported, output, pose_limit=12):
    ...
    result = {'schema': 1, 'species': 'BlueKochappy', 'source_id': 44, 'health': 250,
              'motions': report,
              ...
              'materials': 'Rigid bake with approximate_materials=True; TEV stages approximated, source BTI image zero bound.',
              'supported': 'Source BlueKochappy visuals and health 250; deterministic pose bank.',
              'not_implemented': ['P2 FSM/event parity', 'source Purple stun 5s reaction',
