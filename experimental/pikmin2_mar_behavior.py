"""Mar (Puffy Blowhog, EnemyID 29) source-behavior acceptance (#375/#166).

Builds the private batch-3 flying arena (Mar + Hanachirashi + P1 control) via
`experimental.pikmin2_flying_arena.prepare`. The arena stages Mar at
(-150, 30, 1850), about 32 units from the 20-red fixture squad, so the source
sight radius (fp12=275) is satisfied without a behavior-fixture position
override. The native `pc_port/pc_p2_mar.cpp` drives the bounded source FSM
Wait -> Move/Chase -> Attack (InteractFlick wind at the attack KEYEVENT_2) ->
Wait and prints `P2_MAR_*` markers; `validate()` checks only those markers plus
the batch-3 bind/ready lines, and makes no claim about other families.
"""
import argparse
import json
import os
...
MAR_POSITION = (-150.0, 30.0, 1850.0)
SIGHT = 275.0
...
def run(assets, imported, output, exe, seconds=30):
    ...
    override = dict(species='Mar', generator=MAR_ID, arena_default=list(MAR_POSITION),
                    behavior_fixture=None,
                    reason='batch-3 flying arena already stages Mar within fp12=275 of '
                           'the fixture squad; no behavior position override applied',
                    production_placement=False,
                    pose_name_normalization=normalize_pose_names(run_dir))
    (run_dir / 'mar-override.json').write_text(json.dumps(override, indent=2) + '\n')
    ...
    result['capture'] = {k: meta[k] for k in ('executable_sha256', 'elapsed_seconds',
                                              'exit_code', 'timed_out')}
    (run_dir / 'mar-validation.json').write_text(json.dumps(result, indent=2) + '\n')
...
def validate(text, code=0):
    ...
    states = re.findall(r'P2_MAR_STATE generator=375001 state=(\w+)', text)
    blows = [int(n) for n in re.findall(r'P2_MAR_BLOW generator=375001 pikmin=(\d+)', text)]
    positions = [(m.group(1), m.group(2), float(m.group(3)), float(m.group(4)), float(m.group(5)))
                 for m in re.finditer(
        r'P2_MAR_POS generator=375001 state=(\w+) clip=(\w+) phase=([\d.]+) '
        r'x=(-?[\d.]+) z=(-?[\d.]+)', text)]
    spread = 0.0
    if len(positions) >= 2:
    ...
    checks = dict(
        identity=bool(re.search(r'P2_MAR_BIND generator=375001 source_id=29 visual_only=0', text)),
        ready=bool(re.search(r'P2_ENEMY_READY species=Mar native_family=Mar generator=375001 '
    ...
    return dict(passed=all(v for k, v in checks.items()
                ...
                unmeasured=['Fall/Land/Ground/TakeOff/FlyFlick/GroundFlick (stuck-Pikmin '
                            ...
                            'P2 InteractWind receiver (invincible/KokeDamage states)',
                ...
                limitations=['Batch-3 flying arena placement is engineered fixture evidence, '
                             ...
                             'Wind is resolved as an InteractFlick blow/stagger at the source '
                             'attack KEYEVENT_2 frame; the P1 engine has no InteractWind.'])
