"""Private original Impact Site arena: one source Honeywisp and one P1 control."""
import argparse
import hashlib
import json
...
from experimental.pikmin2_qurione_install import install
...
def roster(assets):
    ...
    for identity,kind,xyz in [(203001,'Qurione',(-104.,30.,1816.))]:
        ...
        placements.append(dict(generator=identity,species=kind,native_family='Qurione',position=list(validate_position(row,xyz)),offset=[0,0,0],source_yaw=None,source_yaw_applied=False))
    return header[:20]+struct.pack('>I',len(entries))+b''.join(entries),placements


def prepare(assets,bank,output,expected_source_sha256):
    ...
    installation=install(bank,run,[actors[0]['generator']],expected_source_sha256)
...
if __name__=='__main__':
    ...
    a=p.parse_args();print(prepare(a.assets,a.bank,a.output,a.source_sha256))
